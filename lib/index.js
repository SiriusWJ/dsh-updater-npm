 ...
 * 读取插件配置。
 *
 * 默认值：文档同步关闭；npm 安装「空闲 5 分钟无输出」判卡死、总时长硬上限 60 分钟。
 * 旧的 10 分钟一刀切超时在慢速 registry 上会误杀正常下载（222 MB 依赖树 + 冷缓存
 * 实测撞线），改成 idle/hard 双阈值后，只要 npm 还在输出就不会被杀。
 */
function readPluginConfig() {
  ...
  return {
    ...
    npmIdleMinutes: num(raw.npmIdleMinutes, 5, 1, 120),
...
function runNpmUpdate(installDir, beforeVersion, onProgress = null, uilang, expectUpdate = false, targetVersion = 'latest') {
  return new Promise((resolve) => {
    const own = npmCliFor(installDir)
    const base = ['install', '-g', '@deepseek-ai/dsh@' + targetVersion, '--no-audit', '--no-fund', '--loglevel=http', '--progress=false']
    const cmd = own !== null
...
async function installStaged(installDir, targetVersion, stagingDir, uilang, push) {
  const own = npmCliFor(installDir)
  // 关键：必须带 -g。带 -g 的 --prefix 安装生成与现网一致的「嵌套」布局
  ...
  const base = ['install', '-g', '--prefix', stagingDir, '--no-save', '--no-audit', '--no-fund', '@deepseek-ai/dsh@' + targetVersion]
  const argv = own !== null ? [own.nodeExe, own.npmCli, ...base] : ['npm', ...base]
  const cfg = readPluginConfig()
  ...
  const run = await runInstallCmd(argv, (d) => push(tr(uilang, 'updateStaging', { version: targetVersion }), d), { idleMs, hardMs })
  if (!run.ok) {
