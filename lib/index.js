  ...
  if (platform === 'win32') {
    ...
    if (swap && typeof swap.stagingPkg === 'string') {
      ...
      lines.push(
        ...
        "  if (-not (Test-Path '" + inst.replace(/'/g, "''") + "\\package.json')) { throw 'post-swap check failed' }",
    ...
    lines.push(
      "Log 'relaunching dsh'",
      'Start-Process -FilePath ' + psq(nodeExe) + ' -ArgumentList @(' + argList.map(psq).join(', ') + ') -WorkingDirectory ' + psq(cwd),
      "Log 'relaunch issued'",
    )
    return lines.join('\n')
  }
