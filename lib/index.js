...
/** 把重启脚本写入插件数据目录并以 detached 方式启动（当前进程退出后继续执行）。 */
function launchRestartScript(scriptText) {
  const win = process.platform === 'win32'
  const file = join(pluginDataDir(), 'restart-' + Date.now() + (win ? '.ps1' : '.sh'))
  try {
    writeText(file, scriptText)
    const cmdArgs = win
      ? ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', file]
      : [file]
    const child = spawn(win ? 'powershell.exe' : '/bin/sh', cmdArgs, { stdio: 'ignore', detached: true, shell: false })
    child.unref()
    return { ok: true, script: file }
  } catch (e) {
    return { ok: false, error: String((e && e.message) || e), script: null }
  }
}
...
export function apply(ctx) {
  ...
  const restartNow = async (uilang) => {
    ...
    try {
      ...
      const launched = launchRestartScript(scriptText)
