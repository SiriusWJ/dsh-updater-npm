...
import { readFileSync, existsSync, mkdirSync, writeFileSync, readdirSync, statSync, rmSync, realpathSync, cpSync } from 'node:fs'
...
async function selfUpdateStatus(force = false) {
  ...
  const res = await fetchWithTimeout(`https://registry.npmjs.org/${SELF_PACKAGE_NAME}/latest`, { headers: { Accept: 'application/json' } }, 12000)
...
function resolveProfileName() {
  ...
  if (fromArgv !== null && installedIn(fromArgv)) return fromArgv
...
async function verifyStagedBinary(stagingPkg) {
  ...
  const run = await runInstallCmd([process.execPath, binJs, '--version'], null, 60000)
...
function buildRestartScript(opts) {
  ...
  if (platform === 'win32') {
    ...
    if (wantSwap) {
      ...
      lines.push(
        "if (-not (Test-Path " + psq(join(swap.stagingPkg, 'package.json')) + ") -or -not (Test-Path " + psq(join(swap.stagingPkg, 'lib', 'bin.js')) + ")) { " + (hasResult ? "$result.error = 'staging incomplete'; WriteResult; " : '') + "Log 'staging invalid, aborting (no swap, no restart)'; exit 1 }",
        // 结构一致性：现网是嵌套布局时，暂存包也必须是嵌套的，否则交换会丢掉全部依赖
        "if ((Test-Path " + psq(join(swap.installDir, 'node_modules')) + ") -and -not (Test-Path " + psq(join(swap.stagingPkg, 'node_modules')) + ")) { " + (hasResult ? "$result.error = 'staging layout mismatch'; WriteResult; " : '') + "Log 'staging layout mismatch, aborting (no swap, no restart)'; exit 1 }")
    }
    lines.push(
    ...
    if (wantSwap) {
      ...
      lines.push(
        ...
        "    if (-not (Test-Path " + psq(join(inst, 'package.json')) + ")) { throw 'post-swap check failed' }",
  ...
  if (wantSwap) {
    ...
    lines.push(
      "if [ ! -f " + shq(join(swap.stagingPkg, 'package.json')) + " ] || [ ! -f " + shq(join(swap.stagingPkg, 'lib', 'bin.js')) + " ]; then ERR='staging incomplete'; " + (wantResult ? 'write_result; ' : '') + "log 'staging invalid, aborting (no swap, no restart)'; exit 1; fi",
      "if [ -d " + shq(join(swap.installDir, 'node_modules')) + " ] && [ ! -d " + shq(join(swap.stagingPkg, 'node_modules')) + " ]; then ERR='staging layout mismatch'; " + (wantResult ? 'write_result; ' : '') + "log 'staging layout mismatch, aborting (no swap, no restart)'; exit 1; fi")
  }
  lines.push(
    'sleep 2',
    'PID=' + String(pid),
  ...
  if (wantSwap) {
    ...
    lines.push(
      ...
      '  if mv -f ' + shq(stg) + ' ' + shq(inst) + " 2>/dev/null && [ -f " + shq(join(inst, 'package.json')) + ' ]; then',
