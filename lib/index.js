...
export function apply(ctx) {
  ...
  ctx.effect(() => {
    ...
    let disposed = false
    ;(async () => {
      // locateInstall 返回 { installDir, mode } —— 必须解构出字符串路径，
      // 否则整个对象被当作路径传入 registerDocTools（loadDefineTool 里 join
      // 抛错 → 返回 null → 文档工具静默不注册；症状即「[object Object]」）。
      const located = await locateInstall(ctx)
      if (disposed || located === null) {
