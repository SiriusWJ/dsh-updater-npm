 ...
 *   POST /dsh-updater-npm/restart      重启当前 DSH 实例（Windows/macOS/Linux）
...
import { readFileSync, existsSync, mkdirSync, writeFileSync, readdirSync, statSync, rmSync, realpathSync, cpSync, chmodSync } from 'node:fs'
import { dirname, join, basename, extname, normalize, resolve, sep, delimiter, posix, win32 } from 'node:path'
...
const HOST_I18N = {
  zh: {
    ...
    sourceWarning: '源码树模式（{dir}，v{version}）：将执行 git pull + 依赖安装',
    // 源码树（git）更新
    ...
    updateStagingStalled: 'npm 连续 {min} 分钟无输出，已终止（运行 {sec}）',
    updateStagingTimeout: 'npm 超过总时长上限 {min} 分钟，已终止（运行 {sec}）',
    stagedReady: '✅ 新版 v{version} 已就绪（暂存完成，尚未生效）——点击「重启 DSH」完成更新',
    // 重启当前 DSH 实例（跨平台）
    restartBtn: '重启 DSH',
    restarting: '正在重启 DSH…',
    restartLaunched: '✅ 已触发重启：DSH 即将退出并自动重新启动…',
    restartFail: '重启失败: ',
    restartNotStarted: '重启脚本未启动，部署与暂存包未改动；请手动重启 DSH 后再点一次',
    busyRestart: '一次只允许一个重启操作，请稍候重试',
    deployBrokenWarn: '⚠️ 部署目录不完整，建议「修复部署」',
    repairBtn: '修复部署（重装当前版本）',
    repairRunning: '正在修复部署…（重装当前版本，完成后点击「重启 DSH」生效）',
    repairDone: '✅ 部署已修复——点击「重启 DSH」使其生效',
    repairFail: '部署修复失败: ',
    // 升级安全网（0.1.5 升级实战经验沉淀）
    stagingLayoutMismatch: '暂存目录结构与当前部署不一致（暂存包缺少 node_modules，交换后会丢失全部依赖）。已中止更新：请确认 npm 版本较新后重试。',
    stagingWasteFound: '检测到 {n} 个遗留暂存目录（{mb} MB），通常来自未完成的更新，可在下方一键清理。',
    stagingSwept: '[dsh-updater-npm] 已清理 {n} 个遗留暂存目录，释放 {mb} MB',
    backupCreated: '升级前已备份配置到 {dir}',
    backupSessionsCopied: '会话日志已一并备份（{mb} MB）',
    backupSessionsSkipped: '会话日志 {mb} MB 超过 {limit} MB 未备份；升级后会话会迁移到新格式且不可降级读取',
    backupPruned: '[dsh-updater-npm] 已清理 {n} 个较旧的升级备份',
    backingUp: '正在备份升级前的配置与会话…',
    pluginUpdateRequired: '当前插件 v{current} 不是最新（最新 v{latest}）。升级 DSH 的动作由当前插件代码执行，旧版插件的升级流程本身就有风险，已阻止本次升级——请先更新插件、重启 DSH，再重试（命令见下方）。',
    restartLaunchedSwap: '✅ 已触发重启：DSH 将退出，完成版本交换后自动重启。',
    breakingTitle: '跨版本升级注意',
    breakingKnown: '0.1.5 起：会话格式升级到 V3（不可降级读取）；persona 由 text 改为 prefix/suffix；插件 API 与槽位有变更。',
    breakingGeneric: '跨 {n} 个小版本，建议先看 release notes。',
    releaseNotesLink: '查看 release notes',
    lastRestartOk: '上次重启：已交换并启动 v{version}',
    lastRestartFail: '上次重启：交换失败，已回滚原部署',
    lastRestartPlain: '上次重启：仅重启，未交换版本',
    rollbackKept: '旧版本已保留在 {dir}，确认新版稳定后可手动删除',
    npmResidualFound: '发现 npm 安装残留 {n} 个 / {mb} MB（{dir}），通常是中断的安装留下的，可手动删除',
    cleanupBtn: '清理遗留文件',
    cleanupDone: '已清理 {n} 项，释放 {mb} MB',
    cleanupNone: '没有可清理的遗留文件',
  ...
  en: {
    ...
    sourceWarning: 'Running in source-tree mode ({dir}, v{version}): git pull + dependency install',
    // 源码树（git）更新
    ...
    updateStagingStalled: 'npm produced no output for {min} min — terminated (ran {sec})',
    updateStagingTimeout: 'npm exceeded the {min} min total limit — terminated (ran {sec})',
    stagedReady: '✅ v{version} staged and ready (not live yet) — click "Restart DSH" to finish the update',
    // 重启当前 DSH 实例（跨平台）
    ...
    restartNotStarted: 'The restart script did not start; nothing was changed — restart DSH manually and click again',
    busyRestart: 'Only one restart at a time, please retry later',
    deployBrokenWarn: '⚠️ Deployment directory incomplete — use "Repair deployment"',
    ...
    stagingLayoutMismatch: 'The staged tree does not match this deployment (the staged package has no node_modules, so swapping would lose every dependency). Update aborted — check that npm is recent, then retry.',
    stagingWasteFound: 'Found {n} leftover staging director(y/ies) ({mb} MB), usually from an unfinished update — you can clean them below.',
    stagingSwept: '[dsh-updater-npm] cleaned {n} leftover staging director(y/ies), freed {mb} MB',
    backupCreated: 'Pre-upgrade backup written to {dir}',
    backupSessionsCopied: 'Session logs were backed up too ({mb} MB)',
    backupSessionsSkipped: 'Session logs are {mb} MB (over the {limit} MB limit) and were not backed up; sessions migrate to a new format on first write and cannot be read by older versions',
    backupPruned: '[dsh-updater-npm] pruned {n} older upgrade backup(s)',
    backingUp: 'Backing up the current configuration and sessions…',
    pluginUpdateRequired: 'This plugin is not up to date (running v{current}, latest v{latest}). Updating DSH is performed by the plugin code itself, so an outdated plugin makes the upgrade unsafe — the update was stopped. Update the plugin, restart DSH, then retry (command below).',
    restartLaunchedSwap: '✅ Restart triggered — DSH will exit, swap the version, and relaunch itself.',
    breakingTitle: 'Cross-version upgrade notice',
    breakingKnown: 'Since 0.1.5: session format moves to V3 (no downgrade reads); persona text → prefix/suffix; plugin APIs and slots changed.',
    breakingGeneric: 'This jump spans {n} minor versions — read the release notes first.',
    ...
    rollbackKept: 'The previous version is kept at {dir}; delete it once the new one looks stable',
    npmResidualFound: 'Found {n} npm install leftover(s), {mb} MB ({dir}) — usually from an interrupted install; safe to delete manually',
    cleanupBtn: 'Clean leftovers',
    cleanupDone: 'Cleaned {n} item(s), freed {mb} MB',
    cleanupNone: 'Nothing left to clean',
...
function readJson(file) {
  try {
    // 去掉可能的 UTF-8 BOM：PowerShell 的 Set-Content 默认会写 BOM，
    // 而 JSON.parse 会因此报错（重启脚本写回的 restart-result.json 就是这种情况）。
...
// 插件自身版本 / 自更新检查
//
// 为什么要先检查插件自己：升级 DSH 的动作是由**当前运行的插件代码**执行的。
// 若插件本身是旧版（例如 1.9.0 的 Windows staged 安装缺 -g，交换后会丢掉
// 全部依赖），那么"升级 DSH"这一步本身就是危险的。所以升级前先确认插件是
// 最新版，不是就拦住并给出确切的更新命令。
// ────────────────────────────────────────────────────────────────────────────

/** 本插件包名（自更新检查用的 registry 包）。 */
const SELF_PACKAGE_NAME = 'dsh-updater-npm'
/** 本插件自己的 package.json（lib/ 的上一级，npm 包必然包含）。 */
const SELF_PACKAGE_FILE = fileURLToPath(new URL('../package.json', import.meta.url))

/** 本插件的版本号（读不到返回 null）。 */
function selfVersion() {
  const pj = readJson(SELF_PACKAGE_FILE)
  return pj !== null && typeof pj.version === 'string' ? pj.version : null
}

/** 本插件自更新检查缓存（10 分钟，与 /check 缓存同频）。 */
let selfCheckCache = null

/**
 * 查 npm 上本插件的最新版本。
 * 用 /latest 端点（单个 manifest，比整份 packument 小得多）；失败不抛，返回 error。
 * @param {boolean} force 忽略缓存
 */
async function selfUpdateStatus(force = false) {
  const version = selfVersion()
  if (!force && selfCheckCache !== null && Date.now() - selfCheckCache.at < 10 * 60 * 1000) {
    return { ...selfCheckCache.data, version }
  }
  const res = await fetchWithTimeout(`${registryBase()}/${SELF_PACKAGE_NAME}/latest`, { headers: { Accept: 'application/json' } }, 12000)
  const latest = res.ok && res.json !== null && typeof res.json.version === 'string' ? res.json.version : null
  const data = {
    latest,
    error: latest === null ? (res.error || 'registry unreachable') : null,
  }
  selfCheckCache = { at: Date.now(), data }
  return { ...data, version }
}

/**
 * 当前运行实例所在的 profile 名。
 * 优先从 argv（`--profile x` 或 `dsh web`）取，再用「该 profile 装了本插件」验证，
 * 避免在 tui/sdk 等其它 profile 上给出错误的更新命令。
 */
function resolveProfileName() {
  const profilesRoot = join(dshHome(), 'profiles')
  const installedIn = (name) => existsSync(join(profilesRoot, name, 'node_modules', SELF_PACKAGE_NAME, 'package.json'))
  // 插件也可能只落在**共享**的 profiles/node_modules 里（hoisted / 部分 pnpm 布局）。
  // 那种情况下每个 profile 目录下都探测不到它，逐 profile 判定会全部为假，
  // 于是把 argv 里已经确定的来源丢掉、退回硬编码的 'web'，自更新命令就会指错 profile。
  const inShared = () => existsSync(join(profilesRoot, 'node_modules', SELF_PACKAGE_NAME, 'package.json'))
  const argv = process.argv.slice(2)
  const flagIndex = argv.indexOf('--profile')
  let fromArgv = null
  if (flagIndex >= 0 && typeof argv[flagIndex + 1] === 'string' && argv[flagIndex + 1] !== '') {
    fromArgv = argv[flagIndex + 1]
  } else if (typeof argv[0] === 'string' && /^[a-z0-9_-]+$/.test(argv[0])) {
    fromArgv = argv[0]
  }
  if (fromArgv !== null && (installedIn(fromArgv) || inShared())) return fromArgv
  for (const e of listEntries(profilesRoot)) {
    if (e.isDirectory() && installedIn(e.name)) return e.name
  }
  return fromArgv !== null ? fromArgv : 'web'
}

/** 更新本插件的确切命令。 */
function selfUpdateCommand(latest) {
  const target = typeof latest === 'string' && latest !== '' ? `${SELF_PACKAGE_NAME}@${latest}` : SELF_PACKAGE_NAME
  return `dsh plugin --profile ${resolveProfileName()} add ${target}`
}

/**
 * 是否必须先更新插件才能升级 DSH。
 * 只有「确定存在更新版插件」时才拦（registry 不可达 / 版本未知一律放行），
 * 避免离线或企业内网环境被误锁。
 */
function updateBlockedBySelf(plugin) {
  return plugin !== null && typeof plugin === 'object' && plugin.hasUpdate === true
}

/** 官方文档同步的根目录：$DSH_HOME 或默认 ~/.dsh。 */
function dshHome() {
...
/** npm 日志目录（--logs-dir）：npm 会在这里写 <时间戳>-debug-0.log。 */
function npmLogsDir() {
  return join(pluginDataDir(), 'npm-logs')
}

/**
 * 目录内最新的文件 mtime（毫秒）；目录不存在或无文件返回 0。
 * 用作「子进程是否还活着」的探针之一。
 */
...
/**
 * staged 安装的存活探针：返回一个单调不减的「活动时间戳」。
 *
 * 实测（npm 10.9.8 / Windows）两种静默期都会踩坑：
 *  1) 非 TTY 下 npm 默认几乎不往 stdout 写 → 靠 --loglevel=http 让网络阶段有输出；
 *  2) npm 的 debug 日志文件**可能整场只在退出时落盘**（14 秒的安装里 12 秒都是 0 字节），
 *     所以日志目录本身不足以当心跳 → 必须同时看**文件系统副作用**：
 *     解包阶段 npm 会不停往暂存目录里写文件，node_modules 的 mtime 一直在动。
 *
 * 三路取最大值：logs 目录、暂存根目录、暂存 node_modules（都是浅层 stat，开销可忽略）。
 */
function stagedActivityProbe(stagingDir) {
  return () => Math.max(
    newestMtimeMs(npmLogsDir()),
    newestMtimeMs(stagingDir),
    newestMtimeMs(join(stagingDir, 'node_modules')),
  )
}
 ...
 * 读取插件配置。
 *
 * 默认值：文档同步关闭；npm 安装「空闲 10 分钟无活动」判卡死、总时长硬上限 60 分钟。
 * 旧的 10 分钟一刀切超时在慢速 registry 上会误杀正常下载（222 MB 依赖树 + 冷缓存
 * 实测撞线），改成 idle/hard 双阈值后，只要 npm 还在动就不会被杀。
 * 「还在动」既看子进程 stdout，也看 npm 自己的 debug 日志文件（见 activityProbe）。
 */
function readPluginConfig() {
  ...
  return {
    docsEnabled: !!raw.docsEnabled,
    // 默认 10 分钟：实测 npm 的元数据阶段可以安静很久（stdout 无输出但日志文件在写），
    // 配合 activityProbe 后，只有「连 npm 自己的日志都不动了」才会被判卡死。
    npmIdleMinutes: num(raw.npmIdleMinutes, 10, 1, 120),
    npmTimeoutMinutes: num(raw.npmTimeoutMinutes, 60, 5, 24 * 60),
  }
}

/** 读取文档同步开关。 */
function readDocsConfig() {
  return { docsEnabled: readPluginConfig().docsEnabled }
}

/** 写插件配置（合并写入，避免抹掉 npmIdleMinutes / npmTimeoutMinutes 等其它键）。 */
function writeDocsConfig(cfg) {
  const cur = readJson(docsConfigFile())
  writeJson(docsConfigFile(), { ...(cur !== null && typeof cur === 'object' ? cur : {}), ...cfg })
}

/** npm registry 默认基址（用户没有任何配置时使用）。 */
const DEFAULT_REGISTRY = 'https://registry.npmjs.org'

/** registry 基址缓存（进程内；改了 .npmrc 需重启 DSH 才生效，够用）。 */
let registryBaseCache = null

/** 从一份 npmrc 里取 registry=（只认第一条；跳过注释与空行）。 */
function registryFromNpmrc(file) {
  let text
  try { text = readFileSync(file, 'utf8') } catch (e) { return null }
 ...
 * npm registry 基址。
 *
 * 旧实现把 `https://registry.npmjs.org` 硬编码在检查端点里，完全无视用户配置的镜像
 * （国内镜像站、企业内网 registry 都很常见）：插件去 npmjs 查版本、npm 却从镜像装包。
 * 轻则慢、重则被墙拿到「registry unreachable」，而实际上更新本来是能成功的。
 *
 * 取用顺序按 npm 的常识来：环境变量 → 当前目录 .npmrc → 用户 ~/.npmrc → 默认值。
 * 任何一环取到的值不像 http(s) URL 就继续往下走，保证不会因为配置写坏而失效。
 * ponytail: 不处理 scoped registry（@scope:registry=）与认证配置；要完整语义得问
 * `npm config get registry`，但那需要起子进程，检查路径上不划算。
 */
function registryBase() {
  if (registryBaseCache !== null) return registryBaseCache
  const candidates = [
    process.env.npm_config_registry,
    registryFromNpmrc(join(process.cwd(), '.npmrc')),
    registryFromNpmrc(join(homedir(), '.npmrc')),
  ]
  for (const candidate of candidates) {
    if (typeof candidate !== 'string') continue
    const url = candidate.trim().replace(/\/+$/, '')
    if (/^https?:\/\/[^\s]+$/i.test(url)) {
      registryBaseCache = url
      return url
    }
  }
  registryBaseCache = DEFAULT_REGISTRY
  return registryBaseCache
}
...
function npmCliFor(installDir) {
  ...
  if (modulesDir !== null) {
    ...
    if (existsSync(npmCli)) return { nodeExe: process.execPath, npmCli, nodeRoot, modulesDir }
  }
  // 回退：全局 prefix 的 node_modules 里没有 npm 包（npm 随 Node 装在别处，
  // 如 C:\Program Files\nodejs，而 prefix 是 %APPDATA%\npm）。此时改从 PATH 上
  // 所有 Node 安装根里找 npm CLI：npm-cli.js 是纯 JS，用当前 node 执行即可；
  // `install -g` 的落点由 npm 的全局 prefix 配置（用户级 %APPDATA%\npm）决定，
  // 与 npm 所在安装无关，仍会命中当前运行副本。
  return npmCliFromPathRoots()
}

/** 从 PATH 上所有 Node 安装根（pathNodeRoots）里找 npm CLI，找不到返回 null。 */
function npmCliFromPathRoots() {
  for (const r of pathNodeRoots()) {
    const npmCli = join(r, 'node_modules', 'npm', 'bin', 'npm-cli.js')
    if (existsSync(npmCli)) {
      return { nodeExe: process.execPath, npmCli, nodeRoot: r, modulesDir: join(r, 'node_modules') }
    }
  }
  return null
}
 ...
 * 回退：以 profile 目录为锚点 require.resolve('@deepseek-ai/dsh/package.json') 再
 * realpath（视为 npm-global）。见下方注释：旧的 preset 路径反推已失效，且锚点不能用
 * import.meta.url（link:/junction 安装时它指向包的真实路径，解析不到 DSH）。
...
async function locateInstall(ctx) {
  ...
  // 兜底：从本插件自身位置解析 @deepseek-ai/dsh。
  //
  // 旧实现是「反推 agent preset 路径」，按 preset.path 以
  // `/agent-presets/<id>/agent.cordis.yml` 结尾再剥掉一层 `/config`。
  // 那个布局早已不存在，实测两个真实形态都对不上：
  //   随包发布： <dsh>/node_modules/@deepseek-ai/dsh-agent-presets/presets/<id>/agent.cordis.yml
  //   用户自建： <DSH_HOME>/.agent-presets/<id>/agent.cordis.yml
  // 两者都不含 `/agent-presets/`，endsWith 恒为 false —— 这段兜底是死代码，
  // 一旦 argv[1] 不是 bin.js（打包 / 嵌入式 / 被包装启动）就彻底定位不到安装目录。
  //
  // 换成模块解析，但**锚点不能用 import.meta.url**：
  // link:/file:/junction 安装时 Node 会把模块 URL 解析成包的真实路径
  // （本机 junction 指向 D:\Code\dsh\dsh-updater-npm），从仓库往上走根本没有
  // node_modules，解析必然失败。部署视角下能看见 DSH 的地方是 **profile 目录**
  // （npm 全局 + profile 里的 junction 镜像 / hoisted / pnpm 都适用）。
  // 第二个锚点留给「插件装在 profile 之外」的场景，成功与否都不影响第一个。
  const anchors = [
    join(resolve(dshHome()), 'profiles', resolveProfileName(), 'package.json'),
    SELF_PACKAGE_FILE,
  ]
  for (const anchor of anchors) {
    try {
      const pkgJson = createRequire(anchor).resolve('@deepseek-ai/dsh/package.json')
      const install = realpathSync(dirname(pkgJson))
      if (existsSync(join(install, 'package.json'))) return { installDir: install, mode: 'npm-global' }
    } catch (e) { /* 换下一个锚点 */ }
  }
  return null
...
async function remoteVersionOf() {
  try {
    const res = await fetch(registryBase() + '/@deepseek-ai/dsh/latest', { signal: AbortSignal.timeout(15000) })
...
/**
 * Windows 下借 shell 启动时的参数转义。
 *
 * shell:true 时 Node 只把 argv 用空格拼成命令行、**不做任何转义**（DEP0190 的原话
 * 就是 arguments are not escaped, only concatenated）。于是任何含空格的参数都会被
 * cmd.exe 拆成两个：`--prefix` / `--logs-dir` 落在 "C:\Users\John Smith\…" 这类
 * 用户目录时，npm 会报 `Unknown command: "Smith\…"` —— 与用户名带空格的环境直接相关。
 * 只在「确实要借 shell 跑」时调用；走全路径的调用点（node + npm-cli.js、msiexec.exe）
 * 本来就不经 shell，不受空格影响，保持原样。
 * ponytail: 只补空格与 cmd 元字符的引号；%VAR% 展开未屏蔽（路径里几乎不出现）。
 */
...
/** 需要借 shell 时逐个转义参数；否则原样返回（非 Windows 一律不走 shell）。 */
function shellArgv(args, useShell) {
  return useShell && process.platform === 'win32' ? args.map(quoteWinArg) : args
}
 ...
 */
function runNpmUpdate(installDir, beforeVersion, onProgress = null, uilang, expectUpdate = false, targetVersion = 'latest') {
  return new Promise((resolve) => {
    const own = npmCliFor(installDir)
    const logDir = npmLogsDir()
    try { mkdirSync(logDir, { recursive: true }) } catch (e) { /* 忽略 */ }
    const base = ['install', '-g', '@deepseek-ai/dsh@' + targetVersion, '--no-audit', '--no-fund', '--loglevel=http', '--progress=false', '--logs-dir', logDir]
    const cmd = own !== null
      ? { kind: 'own', argv: [own.nodeExe, own.npmCli, ...base] }
      : { kind: 'path', argv: ['npm', ...base] }
    opLogNote('$ ' + cmd.argv.map((a) => (/\s/.test(a) ? '"' + a + '"' : a)).join(' '))
    const useShell = cmd.kind === 'path' && process.platform === 'win32'
    let child
    try {
      child = spawn(cmd.argv[0], shellArgv(cmd.argv.slice(1), useShell), {
    ...
    const armIdle = () => {
      ...
      idleTimer = setTimeout(() => {
        timedOut = 'idle'
        opLogNote('[watchdog] 连续 ' + formatSec(cfg.npmIdleMinutes * 60) + ' 无输出，判定卡死，终止进程')
        try { child.kill() } catch (e) { /* already gone */ }
      }, cfg.npmIdleMinutes * 60 * 1000)
    }
    armIdle()
    const timer = setTimeout(() => {
      timedOut = 'hard'
      opLogNote('[watchdog] 达到总时长上限 ' + String(cfg.npmTimeoutMinutes) + ' 分钟，终止进程')
      try { child.kill() } catch (e) { /* already gone */ }
    }, cfg.npmTimeoutMinutes * 60 * 1000)
    // 同样的存活探针：npm 的 debug 日志可能整场只在退出时落盘，所以也看部署目录写入
    const inPlaceProbe = () => Math.max(
      newestMtimeMs(logDir),
      newestMtimeMs(installDir),
      newestMtimeMs(join(installDir, 'node_modules')),
    )
    let lastProbe = inPlaceProbe()
    let lastNoteAt = Date.now()
    const probeEveryMs = Math.max(300, Math.min(3000, Math.floor(cfg.npmIdleMinutes * 60 * 1000 / 3)))
    const probeTimer = setInterval(() => {
      const value = inPlaceProbe()
      if (value === lastProbe) return
      lastProbe = value
      lastOutputAt = Date.now()
      armIdle()
      if (Date.now() - lastNoteAt >= 60000) {
        lastNoteAt = Date.now()
        opLogNote('[heartbeat] 进程仍在工作（日志文件持续更新），已运行 ' + formatSec((Date.now() - startedAt) / 1000))
      }
    }, probeEveryMs)
    const pushProgress = () => {
      if (onProgress) {
    ...
    child.stdout.on('data', (chunk) => { const text = chunk.toString(); stdout = (stdout + text).slice(-16384); opLogFeed(text); lastOutputAt = Date.now(); armIdle(); pushProgress() })
    child.stderr.on('data', (chunk) => { const text = chunk.toString(); stderr = (stderr + text).slice(-16384); opLogFeed(text); lastOutputAt = Date.now(); armIdle(); pushProgress() })
    ...
    child.on('close', (code) => {
      ...
      clearInterval(heartbeat)
      opLogFlush()
      opLogNote('[exit] code=' + String(code) + '，已运行 ' + formatSec((Date.now() - startedAt) / 1000))
      ...
      resolve({
        ...
        error: code !== 0
          ? (timedOut === 'idle'
              ? tr(uilang, 'updateStagingStalled', { min: String(cfg.npmIdleMinutes), sec: formatSec((Date.now() - startedAt) / 1000) })
              : timedOut === 'hard'
                ? tr(uilang, 'updateStagingTimeout', { min: String(cfg.npmTimeoutMinutes), sec: formatSec((Date.now() - startedAt) / 1000) })
                : (stderr.trim().slice(-400) || tr(uilang, 'npmExitCode', { code: String(code) })))
          : null,
        hint,
      })
    })
...
function runGit(dir, args, onProgress = null) {
  return new Promise((resolve) => {
    ...
    const logIt = onProgress !== null
    child.stdout.on('data', (chunk) => { stdout = (stdout + chunk.toString()).slice(-8192); if (logIt) opLogFeed(chunk.toString()); push() })
    child.stderr.on('data', (chunk) => { stderr = (stderr + chunk.toString()).slice(-8192); if (logIt) opLogFeed(chunk.toString()); push() })
    child.on('error', (e) => { clearTimeout(timer); resolve({ ok: false, code: -1, stdout, stderr: String((e && e.message) || e) }) })
    child.on('close', (code) => { clearTimeout(timer); if (logIt) opLogFlush(); resolve({ ok: code === 0, code, stdout: stdout.trim(), stderr: stderr.trim() }) })
  })
}
 ...
 *  复用 runInstallCmd 的空闲/硬上限看门狗与日志留痕（旧版这里是第三个 10 分钟硬超时）。 */
async function runDepsInstall(root, onProgress, uilang) {
  const usePnpm = existsSync(join(root, 'pnpm-lock.yaml')) || existsSync(join(root, 'pnpm-workspace.yaml'))
  const cmd = usePnpm ? 'pnpm' : 'npm'
  const cfg = readPluginConfig()
  opLogNote('$ ' + cmd + ' install   (' + root + ')')
  const run = await runInstallCmd([cmd, 'install'], onProgress, {
    cwd: root,
    shell: process.platform === 'win32',
    env: { ...process.env, CI: 'true' },
    idleMs: cfg.npmIdleMinutes * 60 * 1000,
    hardMs: cfg.npmTimeoutMinutes * 60 * 1000,
  })
  if (run.ok) return { ...run, cmd }
  const sec = formatSec((run.elapsedMs || 0) / 1000)
  const error = run.timedOut === 'idle'
    ? tr(uilang, 'updateStagingStalled', { min: String(cfg.npmIdleMinutes), sec })
    : run.timedOut === 'hard'
      ? tr(uilang, 'updateStagingTimeout', { min: String(cfg.npmTimeoutMinutes), sec })
      : (run.detail || run.error || null)
...
// ────────────────────────────────────────────────────────────────────────────
// 操作日志缓冲区：把子进程输出按行留痕，供客户端滚动面板增量读取。
// 只保留最近 OP_LOG_MAX_LINES 行 / OP_LOG_MAX_BYTES 字节，避免长安装吃内存。
// ────────────────────────────────────────────────────────────────────────────
const OP_LOG_MAX_LINES = 800
const OP_LOG_MAX_BYTES = 128 * 1024
const opLog = { lines: [], dropped: 0 }
let opLogCarry = ''
...
function opLogAppend(raw) {
  const text = raw.length > 2000 ? raw.slice(0, 2000) + ' …' : raw
  opLog.lines.push(text)
  let bytes = 0
  for (const line of opLog.lines) bytes += line.length + 1
  while (opLog.lines.length > OP_LOG_MAX_LINES || (bytes > OP_LOG_MAX_BYTES && opLog.lines.length > 1)) {
    bytes -= opLog.lines.shift().length + 1
    opLog.dropped += 1
  }
}

/** 把子进程输出块按行喂进日志（剥 ANSI 转义、处理 \r 覆盖行）。 */
function opLogFeed(chunk) {
  const text = String(chunk).replace(/\u001b\[[0-9;]*[A-Za-z]/g, '')
  const parts = (opLogCarry + text).split(/\n/)
  opLogCarry = parts.pop()
  for (const part of parts) {
    const clean = part.split('\r').pop().trimEnd()
    if (clean !== '') opLogAppend(clean)
  }
}

/** 收尾：把没有换行的残行也记下来。 */
function opLogFlush() {
  const clean = opLogCarry.split('\r').pop().trim()
  opLogCarry = ''
  if (clean !== '') opLogAppend(clean)
}

/** 记一行非子进程输出（阶段标题、命令行等）。 */
function opLogNote(text) {
  opLogFlush()
  opLogAppend(text)
}

/**
 * 运行日志落盘（`plugin-data/dsh-updater-npm/last-run.log`）。
 * 界面上不再有滚动日志面板（1.12.3 按用户要求移除），但失败留痕仍然必要——
 * 出问题时可以直接看这个文件，而不用让界面变吵。
 */
function opLogDump(reason) {
  try {
    const head = '[' + new Date().toISOString() + '] ' + String(reason || '') + '\n'
    writeText(join(pluginDataDir(), 'last-run.log'), head + opLog.lines.join('\n') + '\n')
  } catch (e) { /* 忽略 */ }
}

/**
 * 增量快照：total 为累计行号游标（含已裁剪），lines 为序号 ≥ since 的行。
 * 客户端把上次的 total 作为下次的 since，即可只取增量。
 */
function opLogSince(since) {
  const from = Number.isFinite(since) && since > 0 ? Math.floor(since) : 0
  const start = Math.max(0, from - opLog.dropped)
  return { total: opLog.dropped + opLog.lines.length, dropped: opLog.dropped, lines: opLog.lines.slice(start) }
}

/**
 * 执行一条安装命令（npm / winget / msiexec），流式回传输出并留痕。
 *
 * 超时策略（1.12 起，旧版是 10 分钟一刀切）：
 *   idleMs —— 连续无任何输出才判定卡死（默认 5 分钟）；
 *   hardMs —— 总时长硬上限（默认 60 分钟）。
 * 慢速 registry 上 staged 安装纯粹是「下载慢」而不是「卡死」，旧策略会误杀。
 */
function runInstallCmd(argv, onProgress, limits = {}) {
  const idleMs = Number.isFinite(limits.idleMs) ? limits.idleMs : 5 * 60 * 1000
  const hardMs = Number.isFinite(limits.hardMs) ? limits.hardMs : 60 * 60 * 1000
  return new Promise((resolve) => {
    // Windows 上裸命令名（如 'npm'）是 .cmd/.ps1 shim，CreateProcess 无法直接
    // 执行且 Node spawn 不做 PATHEXT 解析（会 ENOENT）——此时借 shell 启动；
    // 带扩展名（msiexec.exe 等）或含路径的命令保持直接 spawn，避免转义问题。
    const cmd0 = String(argv[0] || '').trim()
    const needShell = process.platform === 'win32' && !/[\\/]/.test(cmd0) && !/\.(exe|cmd|bat|ps1)$/i.test(cmd0)
    // needShell：Windows 上裸命令名（npm / pnpm / where…）是 .cmd/.ps1 shim，
    //   Node spawn 不做 PATHEXT 解析会直接 ENOENT，此时借 shell 启动；
    // limits.shell：调用方显式要求（如源码树里跑 npm / pnpm install）。
    // 借了 shell 就必须自己转义参数（见 quoteWinArg），否则带空格路径会被 cmd 拆开。
    const useShell = needShell || limits.shell === true
    let child
    try {
      child = spawn(argv[0], shellArgv(argv.slice(1), useShell), {
        stdio: ['ignore', 'pipe', 'pipe'],
        shell: useShell,
        cwd: typeof limits.cwd === 'string' ? limits.cwd : undefined,
        env: limits.env !== undefined ? limits.env : process.env,
      })
    } catch (e) {
    ...
    const armIdle = () => {
      ...
      idleTimer = setTimeout(() => {
        timedOut = 'idle'
        opLogNote('[watchdog] 连续 ' + formatSec(idleMs / 1000) + ' 无输出，判定卡死，终止进程')
        try { child.kill() } catch (e) { /* already gone */ }
      }, idleMs)
    }
    armIdle()
    const hardTimer = setTimeout(() => {
      timedOut = 'hard'
      opLogNote('[watchdog] 达到总时长上限 ' + formatSec(hardMs / 1000) + '，终止进程')
      try { child.kill() } catch (e) { /* already gone */ }
    }, hardMs)
    // 存活探针：子进程 stdout 安静不代表卡死（npm 在非 TTY 下默认几乎不输出，
    // 解包阶段也可能长时间没有 stdout，但 npm 自己一直在写 debug 日志文件）。
    const probe = typeof limits.activityProbe === 'function' ? limits.activityProbe : null
    let lastProbe = probe !== null ? probe() : null
    let lastNoteAt = Date.now()
    // 探针节奏跟着 idle 窗口走，保证在小窗口（测试/极端配置）下也能及时续命
    const probeEveryMs = Math.max(300, Math.min(3000, Math.floor(idleMs / 3)))
    const probeTimer = probe === null ? null : setInterval(() => {
      const value = probe()
      if (value === null || value === undefined || value === lastProbe) return
      lastProbe = value
      lastOutputAt = Date.now()
      armIdle()
      // 静默期也要让面板看到「还活着」，最多每分钟一条
      if (Date.now() - lastNoteAt >= 60000) {
        lastNoteAt = Date.now()
        opLogNote('[heartbeat] 进程仍在工作（日志文件持续更新），已运行 ' + formatSec((Date.now() - startedAt) / 1000))
      }
    ...
    const onChunk = (which) => (chunk) => {
      ...
      if (which === 'stdout') stdout = (stdout + text).slice(-16384)
    ...
    const finish = (payload) => {
      ...
      if (timedOut !== null) opLogNote('[exit] 已被 watchdog 终止（' + (timedOut === 'idle' ? '空闲超时' : '总时长超限') + '），已运行 ' + formatSec(elapsedMs / 1000))
      else opLogNote('[exit] code=' + String(payload.code) + '，已运行 ' + formatSec(elapsedMs / 1000))
      resolve({ ...payload, timedOut, elapsedMs })
    }
    child.on('error', (e) => finish({ ok: false, error: String((e && e.message) || e), detail: (stderr || stdout).trim().slice(-400) }))
    child.on('close', (code) => finish({ ok: code === 0, code, detail: (stderr || stdout).trim().slice(-400), stdout: stdout.trim().slice(-600), stderr: stderr.trim().slice(-600) }))
  })
}
...
async function installPwshMsi(uilang, push) {
  ...
  try {
    ...
    const msi = await runInstallCmd(['msiexec', '/i', tmp, '/qn', '/norestart'], (d) => push(tr(uilang, 'pwshMsiInstall'), d), { idleMs: 10 * 60 * 1000 })
...
// ────────────────────────────────────────────────────────────────────────────
// 升级安全网（0.1.5 跨版本升级实战沉淀）
//
// 下面这几块各自对应一次真实故障：
//   1) stagingLayoutMatches —— 0.1.2→0.1.5 实测到：暂存包用「不带 -g」的
//      npm install 得到的是提升（hoisted）布局，而现网是 npm 全局的嵌套布局，
//      交换脚本只换包目录 → 新部署丢掉全部依赖。必须在交换前判定结构一致性。
//   2) sweepStaleStaging —— 实测发现 9/4 一次未完成的更新留下 222 MB 暂存目录，
//      因为没有 pending-swap 标记而永远不会被回收。
//   3) createUpgradeBackup —— 跨版本升级前必须留配置/会话快照（会话格式 V3
//      迁移后不可降级读取），0.1.5 升级时全靠人工备份才发现 persona 破坏性变更。
//   4) 重启结果 + 激活地址 —— 0.1.5 起 Web 鉴权 cookie 按「本次激活」签名，
//      重启后必须用新地址重新授权，旧流程会把这段输出丢掉。
// ────────────────────────────────────────────────────────────────────────────

/** 暂存目录过期阈值：未被引用的暂存目录超过此时长即视为遗留垃圾。 */
const STAGING_STALE_MS = 6 * 60 * 60 * 1000
/** 一次性脚本（finisher/restart）过期阈值。 */
const SCRIPT_STALE_MS = 7 * 24 * 60 * 60 * 1000
/** 升级备份里附带会话日志的体积上限，超过只记录路径不复制。 */
const BACKUP_SESSIONS_LIMIT = 256 * 1024 * 1024
/** 升级备份保留份数。 */
const BACKUP_KEEP = 5

/** 已知的跨版本破坏性变更（key = 目标版本的 major.minor）。 */
const BREAKING_NOTES = {
  '0.1.5': { after: '0.1.2', key: 'breakingKnown' },
}

/** 目录体积缓存，避免每次检查都遍历几万个文件。 */
...
function formatMb(bytes) {
  ...
  return String(Math.round((n / 1048576) * 10) / 10)
}
...
/** 带缓存的目录体积。 */
function dirSizeCached(dir, ttlMs = 5 * 60 * 1000) {
  const hit = dirSizeCache.get(dir)
  if (hit !== undefined && Date.now() - hit.at < ttlMs) return hit.bytes
  const bytes = dirSize(dir)
  dirSizeCache.set(dir, { at: Date.now(), bytes })
  return bytes
}

/** 读取目录条目（失败返回空数组）。 */
function listEntries(root) {
  try { return readdirSync(root, { withFileTypes: true }) } catch (e) { return [] }
}
...
/**
 * 暂存包的目录结构是否与现网部署一致——交换安全的前提。
 * 现网若是「包内自带 node_modules」的 npm 全局嵌套布局，暂存包也必须是嵌套的。
 */
function stagingLayoutMatches(installDir, stagingPkg) {
  if (!existsSync(join(installDir, 'node_modules'))) return true // 非常规布局：无从校验
  return existsSync(join(stagingPkg, 'node_modules'))
}

/** 执行暂存包自带的 bin.js --version，确认它真的能跑起来（依赖可解析）。 */
async function verifyStagedBinary(stagingPkg) {
  const binJs = join(stagingPkg, 'lib', 'bin.js')
  if (!existsSync(binJs)) return { ok: false, version: null, error: 'lib/bin.js missing' }
  // 第三参必须是 limits 对象。旧写法传的是数字 60000，而签名早已改成
  // (argv, onProgress, limits = {})：数字上的 .idleMs / .hardMs 都是 undefined，
  // 于是 60 秒上限静默失效、落到默认 5/60 分钟——暂存包半坏时这一步会挂满一小时。
  const run = await runInstallCmd([process.execPath, binJs, '--version'], null, { idleMs: 30000, hardMs: 60000 })
  if (!run.ok) return { ok: false, version: null, error: run.detail || run.error || 'staged binary failed' }
  const version = String(run.stdout || '').trim().split(/\r?\n/).filter(Boolean).pop() || null
  return version === null
    ? { ok: false, version: null, error: 'no version output' }
    : { ok: true, version, error: null }
}

/** 未被 pending-swap 引用的暂存目录（遗留垃圾候选）。 */
function listStagingWaste() {
  const pending = readPendingSwap()
  const keep = pending !== null && typeof pending.stagingDir === 'string' ? resolve(pending.stagingDir) : null
  const out = []
  for (const e of listEntries(pluginDataDir())) {
    if (!e.isDirectory() || !/^(staging|repair)-\d+$/.test(e.name)) continue
    const full = join(pluginDataDir(), e.name)
    if (keep !== null && resolve(full) === keep) continue
    out.push(full)
  }
  return out
}

/** 遗留暂存目录的体积汇总（用于 UI 提示，不删除）。 */
function summarizeStagingWaste() {
  const dirs = listStagingWaste()
  if (dirs.length === 0) return null
  let bytes = 0
  for (const d of dirs) bytes += dirSizeCached(d)
  return { count: dirs.length, bytes, dirs }
}

/**
 * 清理遗留暂存目录与过期的一次性脚本。
 * @param {{force?: boolean}} options force=true 时忽略时间阈值（用户手动清理）
 * @returns {{count: number, bytes: number}}
 */
function sweepStaleStaging(options = {}) {
  const force = options.force === true
  const now = Date.now()
  let count = 0
  let bytes = 0
  for (const dir of listStagingWaste()) {
    const age = now - mtimeOf(dir)
    if (!force && age < STAGING_STALE_MS) continue
    bytes += dirSize(dir)
    dirSizeCache.delete(dir)
    try { rmSync(dir, { recursive: true, force: true }); count += 1 } catch (e) { /* 忽略 */ }
  }
  for (const e of listEntries(pluginDataDir())) {
    if (!e.isFile() || !/^(finisher|restart)-\d+\.(ps1|sh)$/.test(e.name)) continue
    const full = join(pluginDataDir(), e.name)
    if (!force && now - mtimeOf(full) < SCRIPT_STALE_MS) continue
    try { rmSync(full, { force: true }) } catch (e) { /* 忽略 */ }
  }
  return { count, bytes }
}

/**
 * 用户主动清理（POST /dsh-updater-npm/cleanup）：
 * 遗留暂存目录/过期脚本 + 中断安装留下的 npm 残留目录。
 * npm 残留只在「超过 1 小时未变动」时才删，避免打断正在进行的安装。
 * @returns {{staging: number, bytes: number, residuals: number, residualBytes: number}}
 */
function cleanupLeftovers(installDir) {
  const swept = sweepStaleStaging({ force: true })
  let residuals = 0
  let residualBytes = 0
  if (typeof installDir === 'string' && installDir !== '') {
    for (const item of listNpmResiduals(installDir)) {
      if (item.ageMs < 60 * 60 * 1000) continue
      try {
        rmSync(item.dir, { recursive: true, force: true })
        dirSizeCache.delete(item.dir)
        residuals += 1
        residualBytes += item.bytes
      } catch (e) { /* 忽略 */ }
    }
  }
  return { staging: swept.count, bytes: swept.bytes, residuals, residualBytes }
}

/**
 * npm 全局安装被中断时留下的 .<name>-<hash> 残留目录（实测有 65 MB 级）。
 * 只做检测与提示，不自动删除：删除时机应由用户决定。
 */
function listNpmResiduals(installDir) {
  const scopeDir = dirname(installDir)
  const out = []
  for (const e of listEntries(scopeDir)) {
    if (!e.isDirectory() || !/^\..+-[0-9a-zA-Z]{6,}$/.test(e.name)) continue
    const full = join(scopeDir, e.name)
    out.push({ dir: full, bytes: dirSizeCached(full), ageMs: Date.now() - mtimeOf(full) })
  }
  return out
}

/** 残留目录汇总。 */
function summarizeNpmResiduals(installDir) {
  const items = listNpmResiduals(installDir)
  if (items.length === 0) return null
  let bytes = 0
  for (const it of items) bytes += it.bytes
  return { count: items.length, bytes, dirs: items.map((it) => it.dir) }
}

/**
 * 回滚点目录：staged 交换把旧部署改名留下的 `<leaf>.old-<yyyymmddhhmmss>`。
 * 只列出**版本与当前运行版本不同**的那些——正在跑的版本永远不能当回滚点删掉。
 */
function listRollbackDirs(installDir, currentVersion) {
  const scopeDir = dirname(installDir)
  const leaf = basename(installDir)
  const pattern = new RegExp('^' + leaf.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\.old-\\d{14}$')
  const out = []
  for (const e of listEntries(scopeDir)) {
    ...
    if (version !== null && currentVersion !== null && version === currentVersion) continue
    out.push({ path: full, name: e.name, version, bytes: dirSizeCached(full) })
  }
  return out
}

/** 回滚点汇总（无则 null）。 */
function summarizeRollbackDirs(installDir, currentVersion) {
  const dirs = listRollbackDirs(installDir, currentVersion)
  if (dirs.length === 0) return null
  let bytes = 0
  for (const d of dirs) bytes += d.bytes
  return {
    count: dirs.length,
    bytes,
    dirs: dirs.map((d) => ({ name: d.name, version: d.version, bytes: d.bytes })),
  }
}

/** 删除回滚点（仅版本与当前不同的），返回释放数量与体积。 */
function removeRollbackDirs(installDir, currentVersion) {
  let files = 0
  let bytes = 0
  for (const d of listRollbackDirs(installDir, currentVersion)) {
    bytes += d.bytes
    dirSizeCache.delete(d.path)
    try { rmSync(d.path, { recursive: true, force: true }); files += 1 } catch (e) { /* 忽略 */ }
  }
  return { files, bytes }
}

/** 升级备份根目录。 */
function upgradeBackupsRoot() {
  return join(dshHome(), 'upgrade-backups')
}
/** 复制文件/目录（存在才复制，失败返回 false，绝不抛）。 */
...
/**
 * 升级前快照：settings / credentials / .agent-presets / 各 profile 的组合与锁文件；
 * 会话日志按体积门槛决定是否一并复制（迁移不可降级，备份价值高）。
 * @returns {{dir: string|null, copied: string[], sessions: string, sessionsBytes: number}}
 */
function createUpgradeBackup(fromVersion, toVersion) {
  const home = dshHome()
  const stamp = new Date().toISOString().replace(/[:.]/g, '-').replace('T', '-').slice(0, 19)
  const dir = join(upgradeBackupsRoot(), 'dsh-' + (fromVersion || 'unknown') + '-to-' + (toVersion || 'unknown') + '-' + stamp)
  const copied = []
  // 备份里含 .credentials.yaml 与整个 sessions（会话正文），必须只有本用户可读。
  // 旧写法用默认 umask（POSIX 上通常 0755）建目录 → 多用户主机上同机其他账号
  // 可以直接读走凭据副本。这里给备份根与本次目录都收紧到 0700；Windows 上
  // mode 被忽略，改由 ACL 保证（本来就是用户私有目录）。
  try { chmodSync(upgradeBackupsRoot(), 0o700) } catch (e) { /* 不存在或非 POSIX：忽略 */ }
  try { mkdirSync(dir, { recursive: true, mode: 0o700 }) } catch (e) {
    return { dir: null, copied, sessions: 'absent', sessionsBytes: 0 }
  }
  try { chmodSync(dir, 0o700) } catch (e) { /* 已存在时补一刀 */ }
  const singles = [
    ['settings.yaml', join(home, 'settings.yaml')],
    ['.credentials.yaml', join(home, '.credentials.yaml')],
    ['.agent-presets', join(home, '.agent-presets')],
  ]
  for (const [name, src] of singles) if (copyInto(src, join(dir, name))) copied.push(name)
  const profilesRoot = join(home, 'profiles')
  const configFiles = ['package.json', 'cordis.yml', 'cordis.patch.yml', 'pnpm-lock.yaml', 'pnpm-workspace.yaml', '.npmrc']
  for (const e of listEntries(profilesRoot)) {
    if (!e.isDirectory()) continue
    for (const f of configFiles) {
      const rel = join('profiles', e.name, f)
      if (copyInto(join(profilesRoot, e.name, f), join(dir, rel))) copied.push(rel)
    }
  }
  ...
  if (existsSync(sessionsDir)) {
    ...
    sessions = sessionsBytes <= BACKUP_SESSIONS_LIMIT && copyInto(sessionsDir, join(dir, 'sessions')) ? 'copied' : 'skipped'
  }
  writeJson(join(dir, 'upgrade.json'), {
  ...
  return { dir, copied, sessions, sessionsBytes }
}

/** 只保留最近 N 份升级备份，返回清理数量。 */
function pruneUpgradeBackups(keep = BACKUP_KEEP) {
  const dirs = listEntries(upgradeBackupsRoot())
    .filter((e) => e.isDirectory() && e.name.startsWith('dsh-'))
    .map((e) => {
      const full = join(upgradeBackupsRoot(), e.name)
      return { full, mtime: mtimeOf(full) }
    })
    .sort((a, b) => b.mtime - a.mtime)
  let count = 0
  for (const d of dirs.slice(keep)) {
    try { rmSync(d.full, { recursive: true, force: true }); count += 1 } catch (e) { /* 忽略 */ }
  }
  return { count }
}

/** 重启结果文件（由重启脚本写入，宿主读取后展示）。 */
function restartResultFile() {
  return join(pluginDataDir(), 'restart-result.json')
}

/** 重启进程输出日志。 */
function restartOutFile() {
  return join(pluginDataDir(), 'web-restart.out.log')
}

/** 重启进程错误日志。 */
function restartErrFile() {
  return join(pluginDataDir(), 'web-restart.err.log')
}

/** 读取上次重启结果（无则 null）。 */
function readRestartResult() {
  const r = readJson(restartResultFile())
  if (r === null || typeof r !== 'object') return null
  return {
    at: typeof r.at === 'string' ? r.at : null,
    swapped: r.swapped === true,
    swapOk: r.swapOk === true,
    version: typeof r.version === 'string' ? r.version : null,
    oldDir: typeof r.oldDir === 'string' ? r.oldDir : null,
    error: typeof r.error === 'string' ? r.error : null,
  }
}

/**
 * 目标版本相对当前版本的破坏性变更提示（无则 null）。
 * 已知的跨小版本区间命中 BREAKING_NOTES；否则小版本跨度 ≥2 时给通用提示。
 */
function breakingNoteFor(fromVersion, toVersion) {
  const to = parseVersion(toVersion)
  if (to === null) return null
  const from = parseVersion(fromVersion)
  const hit = BREAKING_NOTES[to.nums.join('.')]
  if (hit !== undefined) {
    if (from !== null && compareVersions(fromVersion, hit.after) > 0) return null
    return { key: hit.key, params: {} }
  }
  if (from === null) return null
  const span = (to.nums[0] - from.nums[0]) * 100 + (to.nums[1] - from.nums[1])
  return span >= 2 ? { key: 'breakingGeneric', params: { n: span } } : null
}

/** 目标版本的官方 release notes 链接（版本号非法则 null）。 */
function releaseNotesUrl(version) {
  if (typeof version !== 'string' || parseVersion(version) === null) return null
  return 'https://github.com/' + DOCS_REPO + '/releases/tag/dsh-v' + version
}
 ...
 * 用 --install-strategy=nested 强制「嵌套布局」：所有依赖装进
 * <staging>/node_modules/@deepseek-ai/dsh/node_modules，主包自包含 ——
 * 否则 npm 装成「提升布局」（依赖散布在 staging 顶层），swap 只搬主包时
 * 新部署就成了一个没有依赖的空壳，启动即崩（致命缺陷）。
 * @returns {Promise<{ok, stagingPkg, version, error}>}
 */
/**
 * staged 安装的 npm argv。
 *
 * 必须带 -g：带 -g 的 --prefix 安装生成与现网一致的「嵌套」布局
 * （<prefix>/node_modules/@deepseek-ai/dsh/node_modules/…，包目录自包含），
 * 交换后依赖完整；不带 -g 只会得到提升（hoisted）布局，而交换只搬包目录，
 * 结果就是新部署丢掉全部依赖（0.1.2→0.1.5 升级实测到的真实故障）。
 *
 * 另外三个参数也是必需的（1.12.1 惨痛经验）：
 *   --loglevel=http —— 非 TTY 下 npm 默认几乎不输出，界面只能看到「空白 + 超时」；
 *                      http 级日志把每个请求/阶段打到 stdout，进度面板才有内容，
 *                      看门狗也才有真实心跳（实测：默认级别 5 分钟 stdout 一行没有）。
 *   --progress=false —— 关掉 TTY 进度条，避免 \r 刷屏。
 *   --logs-dir      —— 让 npm 把完整 debug 日志写进插件目录（供探活参考；注意实测
 *                      该文件在 Windows 上可能整场只在退出时落盘，所以真正兜底的是
 *                      stagedActivityProbe 里的文件系统信号）。
 */
function stagedInstallArgv(stagingDir, targetVersion, own, logsDir) {
  const base = [
    'install', '-g', '--prefix', stagingDir, '--no-save', '--no-audit', '--no-fund',
    '--loglevel=http', '--progress=false', '--logs-dir', logsDir,
    '@deepseek-ai/dsh@' + targetVersion,
  ]
  return own !== null ? [own.nodeExe, own.npmCli, ...base] : ['npm', ...base]
}

async function installStaged(installDir, targetVersion, stagingDir, uilang, push) {
  const own = npmCliFor(installDir)
  const cfg = readPluginConfig()
  const idleMs = cfg.npmIdleMinutes * 60 * 1000
  const hardMs = cfg.npmTimeoutMinutes * 60 * 1000
  const logsDir = npmLogsDir()
  try { mkdirSync(logsDir, { recursive: true }) } catch (e) { /* 忽略 */ }
  const argv = stagedInstallArgv(stagingDir, targetVersion, own, logsDir)
  opLogNote('$ ' + argv.map((a) => (/\s/.test(a) ? '"' + a + '"' : a)).join(' '))
  opLogNote('[plan] 空闲超时 ' + String(cfg.npmIdleMinutes) + ' 分钟；总时长上限 ' + String(cfg.npmTimeoutMinutes) + ' 分钟（可在 ' + docsConfigFile() + ' 调整）')
  opLogNote('[plan] 探活：子进程 stdout + npm 日志目录 ' + logsDir + ' + 暂存目录写入')
  const run = await runInstallCmd(argv, (d) => push(tr(uilang, 'updateStaging', { version: targetVersion }), d), {
    idleMs,
    hardMs,
    activityProbe: stagedActivityProbe(stagingDir),
  })
  if (!run.ok) {
    const elapsed = formatSec((run.elapsedMs || 0) / 1000)
    const reason = run.timedOut === 'idle'
      ? tr(uilang, 'updateStagingStalled', { min: String(cfg.npmIdleMinutes), sec: elapsed })
      : run.timedOut === 'hard'
        ? tr(uilang, 'updateStagingTimeout', { min: String(cfg.npmTimeoutMinutes), sec: elapsed })
        : (run.detail || run.error || 'npm')
    return { ok: false, error: tr(uilang, 'updateStagingFail', { err: reason }) }
  }
  ...
  // 交换前校验 1：暂存包结构必须与现网一致（嵌套 vs 提升），否则交换等于拆掉依赖
  if (!stagingLayoutMatches(installDir, stagingPkg)) {
    return { ok: false, error: tr(uilang, 'stagingLayoutMismatch') }
  }
  // 交换前校验 2：暂存包必须真的能执行（依赖可解析），否则不写「待交换」标记
  push(tr(uilang, 'updateStagingVerify'), '')
  const verified = await verifyStagedBinary(stagingPkg)
  if (!verified.ok) {
    return { ok: false, error: tr(uilang, 'updateStagingFail', { err: verified.error || 'staged binary failed' }) }
  }
  const pj = readJson(join(stagingPkg, 'package.json'))
  const version = pj !== null && typeof pj.version === 'string' ? pj.version : verified.version
  return { ok: true, stagingPkg, version, ranVersion: verified.version }
}

/** 待交换标记文件路径（staged 更新/修复后由「重启 DSH」完成交换）。 */
function pendingSwapFile() {
  return join(pluginDataDir(), 'pending-swap.json')
}

/** 读取待交换标记（无则 null）。 */
function readPendingSwap() {
  const p = readJson(pendingSwapFile())
  if (p !== null && typeof p.installDir === 'string' && typeof p.stagingDir === 'string' && typeof p.stagingPkg === 'string') return p
  return null
}

/** 写入待交换标记（kind: 'update' | 'repair'）。 */
function writePendingSwap(kind, installDir, stagingDir, stagingPkg, version) {
  writeJson(pendingSwapFile(), {
    kind,
    installDir,
    stagingDir,
    stagingPkg,
...
/** 清除待交换标记并删除对应暂存目录（新更新取代 / 重启已触发时调用）。 */
function clearPendingSwap() {
  const p = readPendingSwap()
  try { rmSync(pendingSwapFile(), { force: true }) } catch (e) { /* 忽略 */ }
  if (p !== null && typeof p.stagingDir === 'string') {
    try { rmSync(p.stagingDir, { recursive: true, force: true }) } catch (e) { /* 忽略 */ }
  }
}

// ────────────────────────────────────────────────────────────────────────────
// 重启当前 DSH 实例（跨平台：Windows 用 PowerShell，macOS/Linux 用 /bin/sh）
// ────────────────────────────────────────────────────────────────────────────

/**
 * 生成「重启 DSH」启动脚本（纯函数，便于冒烟测试）。
 * 脚本职责：先校验暂存包（若带 swap）→ 稍等当前 HTTP 响应返回 → 结束当前
 * dsh 进程（先温和，超时强制）→ 若带 swap 则原子替换部署目录（旧版改名备份、
 * 新包移入、校验失败自动恢复旧版）→ 按原启动命令（node + execArgv + 脚本参数
 * + 原 cwd）detached 重新拉起 dsh。日志与输出写入 logFile。
 * @param {{platform, pid, nodeExe, args: string[], cwd, logFile, swap?: {installDir, stagingPkg, stagingDir}}} opts
 * @returns 脚本文本（win32 为 PowerShell，其余为 POSIX sh）
 */
function buildRestartScript(opts) {
  const { platform, pid, nodeExe, args, cwd, logFile, swap, resultFile, outLog, errLog } = opts
  const argList = Array.isArray(args) ? args.map(String) : []
  // 路径拼接跟着**目标平台**走，而不是宿主平台。本函数收 platform 参数，内部原先却用
  // 宿主 path.join —— 生产上恰好 platform === process.platform 才看不出问题，但只要在
  // Windows 上生成 POSIX 脚本（测试 / 工具场景），整条路径会被翻成反斜杠、连开头的
  // "/" 都变成 "\"，脚本随即判「暂存包不完整」并放弃交换。用 P 之后参数才名副其实。
  const P = platform === 'win32' ? win32 : posix
  // 两个平台分支共用：待交换 / 结果文件 / 输出重定向 / 激活地址
  const wantSwap = !!(swap && typeof swap.stagingPkg === 'string')
  const hasResult = typeof resultFile === 'string' && resultFile !== ''
  const wantResult = hasResult
  const wantOut = typeof outLog === 'string' && outLog !== ''
  const wantErr = typeof errLog === 'string' && errLog !== ''
  if (platform === 'win32') {
    // PowerShell 单引号字符串：' → ''。不含双引号包裹 —— 旧实现对外包双引号
    // 会让 Start-Process -FilePath 收到带字面引号的路径而找不到文件。
    const psq = (s) => "'" + String(s).replace(/'/g, "''") + "'"
    // -ArgumentList 数组元素：PS 5.1 把数组拼接成命令行直接交给 CreateProcess，
    // 含空格/括号的元素必须自带双引号，否则被拆成多个参数；元素本身仍须是
    // 合法 token（单引号字符串，避免 {{}}/$ 等被当作 scriptblock/插值）。
    const psa = (s) => {
    ...
    const lines = [
      ...
      "function Log($m) { try { Add-Content -Path " + psq(logFile) + " -Value ((Get-Date -Format o) + ' ' + $m) } catch {} }",
      "Log 'restart launcher started'",
    ]
    if (hasResult) {
      lines.push(
        "$result = [ordered]@{ at = (Get-Date -Format o); swapped = $false; swapOk = $false; version = " + psq(wantSwap && swap.version ? swap.version : '') + "; oldDir = ''; error = '' }",
        "function WriteResult { try { [IO.File]::WriteAllText(" + psq(resultFile) + ", ($result | ConvertTo-Json -Compress)) } catch {} }")
    }
    if (wantSwap) {
      // 交换前校验（任一不通过就放弃整个操作：此时旧进程还在跑，不会留下不可用的部署）
      lines.push(
        "if (-not (Test-Path " + psq(P.join(swap.stagingPkg, 'package.json')) + ") -or -not (Test-Path " + psq(P.join(swap.stagingPkg, 'lib', 'bin.js')) + ")) { " + (hasResult ? "$result.error = 'staging incomplete'; WriteResult; " : '') + "Log 'staging invalid, aborting (no swap, no restart)'; exit 1 }",
        // 结构一致性：现网是嵌套布局时，暂存包也必须是嵌套的，否则交换会丢掉全部依赖
        "if ((Test-Path " + psq(P.join(swap.installDir, 'node_modules')) + ") -and -not (Test-Path " + psq(P.join(swap.stagingPkg, 'node_modules')) + ")) { " + (hasResult ? "$result.error = 'staging layout mismatch'; WriteResult; " : '') + "Log 'staging layout mismatch, aborting (no swap, no restart)'; exit 1 }")
    }
    lines.push(
      'Start-Sleep -Seconds 2',
      'Get-Process -Id ' + String(pid) + ' -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue',
      'Start-Sleep -Milliseconds 1500',
    )
    if (wantSwap) {
      const inst = swap.installDir
      const stg = swap.stagingPkg
      const sd = swap.stagingDir
      const instQ = psq(inst)
      const leaf = basename(inst)
      lines.push(
        // 原子替换 + 失败恢复：旧目录改名「保留」而不是删除，留出回滚点
        "$old = " + psq(inst) + " + '.old-' + [DateTime]::Now.ToString('yyyyMMddHHmmss')",
        // 改名重试：刚被结束的进程/子进程可能短暂持有句柄（Windows 上实测会遇到）
        '$renamed = $false',
        'for ($i = 1; $i -le 5; $i++) {',
        "  try { Rename-Item -LiteralPath " + instQ + " -NewName (Split-Path $old -Leaf) -ErrorAction Stop; $renamed = $true; break }",
        "  catch { Log ('rename attempt ' + $i + ' failed: ' + $_.Exception.Message); Start-Sleep -Seconds 3 }",
        '}',
        'if ($renamed) {',
        '  try {',
        "    Move-Item -LiteralPath " + psq(stg) + " -Destination " + instQ + " -ErrorAction Stop",
        "    if (-not (Test-Path " + psq(P.join(inst, 'package.json')) + ")) { throw 'post-swap check failed' }",
        ...(hasResult ? ["    $result.swapped = $true", "    $result.swapOk = $true", "    $result.oldDir = $old"] : []),
        "    Log ('swap OK; previous deployment kept at ' + $old)",
        '  } catch {',
        "    Log ('swap failed: ' + $_.Exception.Message + '; restoring the previous deployment')",
        ...(hasResult ? ["    $result.error = $_.Exception.Message"] : []),
        "    Remove-Item -LiteralPath " + instQ + " -Recurse -Force -ErrorAction SilentlyContinue",
        "    Rename-Item -LiteralPath $old -NewName " + psq(leaf) + " -ErrorAction SilentlyContinue",
        "    Log 'restored the previous deployment'",
        '  }',
        '} else {',
        ...(hasResult ? ["  $result.error = 'could not rename the current deployment'"] : []),
        "  Log 'could not rename the current deployment; keeping it (no swap)'",
        '}',
        ...(hasResult ? ['WriteResult'] : []),
        "Remove-Item -LiteralPath " + psq(sd) + " -Recurse -Force -ErrorAction SilentlyContinue",
      )
    }
    const launchLine = 'Start-Process -FilePath ' + psq(nodeExe) + ' -ArgumentList @(' + argList.map(psq).join(', ') + ') -WorkingDirectory ' + psq(cwd) +
      (wantOut ? ' -WindowStyle Hidden -RedirectStandardOutput ' + psq(outLog) : '') +
      (wantErr ? ' -RedirectStandardError ' + psq(errLog) : '')
    lines.push("Log 'relaunching dsh'")
    if (wantOut) lines.push('Remove-Item -LiteralPath ' + psq(outLog) + ' -Force -ErrorAction SilentlyContinue')
    if (wantErr) lines.push('Remove-Item -LiteralPath ' + psq(errLog) + ' -Force -ErrorAction SilentlyContinue')
    lines.push(launchLine, "Log 'relaunch issued'")
    // 不再抓取/弹出「新激活地址」（1.12.3 起按用户要求移除）：
    // 浏览器侧会自行重新鉴权，脚本也不再为此空转轮询 2 分钟。
    if (hasResult) lines.push('WriteResult')
    return lines.join('\n')
  }
  // POSIX（/bin/sh）：单引号转义 ' → '\''；先 SIGTERM 温和退出，30×0.2s 后 SIGKILL
  ...
  const lines = [
    ...
    'SWAPPED=false',
    'SWAPOK=false',
    'VERSION=' + shq(wantSwap && swap.version ? swap.version : ''),
    'OLDDIR=',
    'ERR=',
    "log 'restart launcher started'",
  ]
  if (wantResult) {
    lines.push(
      'RESULT_FILE=' + shq(resultFile),
      'write_result() { printf \'{"at":"%s","swapped":%s,"swapOk":%s,"version":"%s","oldDir":"%s","error":"%s"}\\n\' "$(date -u +%FT%TZ)" "$SWAPPED" "$SWAPOK" "$VERSION" "$OLDDIR" "$ERR" > "$RESULT_FILE" 2>/dev/null; }')
  }
  if (wantSwap) {
    // 交换前校验（不通过就放弃：旧进程仍在运行，不会留下不可用的部署）
    lines.push(
      "if [ ! -f " + shq(P.join(swap.stagingPkg, 'package.json')) + " ] || [ ! -f " + shq(P.join(swap.stagingPkg, 'lib', 'bin.js')) + " ]; then ERR='staging incomplete'; " + (wantResult ? 'write_result; ' : '') + "log 'staging invalid, aborting (no swap, no restart)'; exit 1; fi",
      "if [ -d " + shq(P.join(swap.installDir, 'node_modules')) + " ] && [ ! -d " + shq(P.join(swap.stagingPkg, 'node_modules')) + " ]; then ERR='staging layout mismatch'; " + (wantResult ? 'write_result; ' : '') + "log 'staging layout mismatch, aborting (no swap, no restart)'; exit 1; fi")
  }
  lines.push(
    'sleep 2',
    'PID=' + String(pid),
  ...
  if (wantSwap) {
    ...
    const sd = swap.stagingDir
    lines.push(
      // 旧目录改名保留（回滚点），失败自动恢复；改名重试应对短暂句柄占用
      "OLD='" + inst.replace(/'/g, "'\\''") + ".old-'" + '"$(date +%Y%m%d%H%M%S)"',
      'SD=' + shq(sd),
      'renamed=false',
      'i=0',
      'while [ "$i" -lt 5 ]; do',
      '  if mv -f ' + shq(inst) + ' "$OLD" 2>/dev/null; then renamed=true; break; fi',
      '  i=$((i + 1)); sleep 3',
      'done',
      'if [ "$renamed" = true ]; then',
      '  if mv -f ' + shq(stg) + ' ' + shq(inst) + " 2>/dev/null && [ -f " + shq(P.join(inst, 'package.json')) + ' ]; then',
      '    SWAPPED=true; SWAPOK=true; OLDDIR="$OLD"',
      "    log 'swap OK; previous deployment kept'",
      '  else',
      "    ERR='swap failed'",
      "    log 'swap failed; restoring the previous deployment'",
      '    rm -rf ' + shq(inst),
      '    mv -f "$OLD" ' + shq(inst) + ' 2>/dev/null',
      "    log 'restored the previous deployment'",
      '  fi',
      'else',
      "  ERR='could not rename the current deployment'",
      "  log 'could not rename the current deployment; keeping it (no swap)'",
      'fi',
      ...(wantResult ? ['write_result'] : []),
      'rm -rf "$SD"',
    )
  }
  lines.push(
    "log 'relaunching dsh'",
    'cd ' + shq(cwd) + ' || exit 1',
    'nohup ' + shq(nodeExe) + (argList.length > 0 ? ' ' + argList.map(shq).join(' ') : '') +
      ' >> ' + shq(wantOut ? outLog : logFile) + (wantErr ? ' 2>> ' + shq(errLog) : ' 2>&1') + ' &',
    "log 'relaunch issued'",
  )
  // 不再抓取/弹出「新激活地址」（1.12.3 起按用户要求移除）
  if (wantResult) lines.push('write_result')
  return lines.join('\n')
}

/**
 * 重启引导脚本（node）：**不要**直接用 detached 启动 powershell.exe。
 *
 * 实测（Windows 10 / Node 22）：spawn('powershell.exe', […], { detached: true })
 * 等于 DETACHED_PROCESS，控制台程序会立刻以 exit=0 退出且**脚本一行都不执行**，
 * 而 spawn 不抛错 → 静默假成功。node.exe 不受影响，所以改成：detached 起 node
 * 引导，由引导进程以「普通子进程」方式启动平台脚本。
 */
const RESTART_BOOTSTRAP = [
  "import { spawn } from 'node:child_process'",
  "import { appendFileSync } from 'node:fs'",
  'const [script, logFile, token] = process.argv.slice(2)',
  'const log = (m) => { try { appendFileSync(logFile, new Date().toISOString() + " " + m + "\\n") } catch (e) {} }',
  'log("node bootstrap started " + token)',
  'const win = /\\.ps1$/i.test(script)',
  'const argv = win ? ["-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", script] : [script]',
  'const child = spawn(win ? "powershell.exe" : "/bin/sh", argv, { stdio: "ignore", windowsHide: true })',
  'child.on("error", (e) => log("bootstrap: could not start the restart script: " + String((e && e.message) || e)))',
  'child.on("exit", (code) => log("bootstrap: restart script exited with " + String(code)))',
].join('\n')

/** 写入平台重启脚本 + node 引导，并以 detached node 启动；返回本次启动的唯一 token。 */
function launchRestartScript(scriptText, logFile) {
  const win = process.platform === 'win32'
  const stamp = Date.now()
  const script = join(pluginDataDir(), 'restart-' + stamp + (win ? '.ps1' : '.sh'))
  const boot = join(pluginDataDir(), 'restart-boot-' + stamp + '.mjs')
  const token = 'boot-' + stamp
  try {
    writeText(script, scriptText)
    writeText(boot, RESTART_BOOTSTRAP)
    const child = spawn(process.execPath, [boot, script, logFile, token], { stdio: 'ignore', detached: true, shell: false })
    child.unref()
    return { ok: true, script, bootstrap: boot, token, pid: child.pid }
  } catch (e) {
    return { ok: false, error: String((e && e.message) || e), script: null, token }
  }
}

/** 等引导进程把启动行写进日志（证明脚本真的跑起来了，而不是静默假成功）。 */
async function waitForBootstrap(logFile, token, timeoutMs = 5000) {
  const deadline = Date.now() + timeoutMs
  while (Date.now() < deadline) {
    try {
      if (existsSync(logFile) && readFileSync(logFile, 'utf8').includes(token)) return true
    } catch (e) { /* 忽略 */ }
...
export function apply(ctx) {
  ...
  /** 插件自身版本信息（/check 与 update 共用）。 */
  const pluginInfo = async () => {
    const st = await selfUpdateStatus()
    return {
      name: SELF_PACKAGE_NAME,
      version: st.version,
      latest: st.latest,
      hasUpdate: st.version !== null && st.latest !== null && compareVersions(st.latest, st.version) > 0,
      error: st.error,
      profile: resolveProfileName(),
      updateCommand: selfUpdateCommand(st.latest),
    }
  }
  ...
  const runFullCheck = async () => {
    ...
    if (mode === 'source') {
      ...
      return {
        ...
        releaseUrl: null,
        plugin,
        lastRestart: readRestartResult(),
        stagingWaste: summarizeStagingWaste(),
        npmResiduals: summarizeNpmResiduals(installDir),
    ...
    return {
      ...
      // 跨版本升级提示（breaking 为 {key, params}，由 check() 按请求语言本地化成 breakingText）
      breaking: hasUpdate ? breakingNoteFor(localVersion, remote.version) : null,
      releaseUrl: hasUpdate ? releaseNotesUrl(remote.version) : null,
      plugin,                                  // 插件自身版本 / 是否有新版本 / 更新命令
      lastRestart: readRestartResult(),        // 上次重启结果 + 新激活地址
      pendingSwap: pendingSwapInfo(),          // 已暂存但尚未交换的新版本（重启失败后仍应可见）
      stagingWaste: summarizeStagingWaste(),   // 未被引用的遗留暂存目录
      rollback: summarizeRollbackDirs(installDir, localVersion), // 回滚点（仅列出非当前版本）
      npmResiduals: summarizeNpmResiduals(installDir), // npm 失败安装残留 .<name>-<hash>
  ...
  /**
   * 未完成交换的暂存包（重启没成功时要用它让用户重试，而不是被当垃圾清掉）。
   * 副作用：若标记还在但暂存目录已丢，顺手清掉这个僵尸标记（自愈）。
   */
  const pendingSwapInfo = () => {
    const p = readPendingSwap()
    if (p === null) return null
    const alive = existsSync(join(p.stagingPkg, 'package.json')) && existsSync(join(p.stagingPkg, 'lib', 'bin.js'))
    if (!alive) {
      clearPendingSwap()
      return null
    }
    return {
      kind: typeof p.kind === 'string' ? p.kind : 'update',
      version: typeof p.version === 'string' ? p.version : null,
      stagingPkg: p.stagingPkg,
  ...
  /** 按请求语言把升级提示（跨版本破坏性变更）本地化成可直接展示的文案。 */
  const withSafetyNotes = (data, uilang) => {
    if (!data || !data.ok || !data.breaking || data.breakingText !== undefined) return data
    const breakingText = tr(uilang, data.breaking.key, data.breaking.params || {})
    return { ...data, breakingText, breaking: undefined }
  }

  const localize = (data, uilang) => withSafetyNotes(withSourceWarning(data, uilang), uilang)

  const check = async (uilang) => {
    try {
      const now = Date.now()
      if (cached !== null && now - cached.at < 10 * 60 * 1000) {
        return { ...localize(cached.data, uilang), cached: true }
      }
      if (checking) {
        if (cached !== null) return { ...localize(cached.data, uilang), cached: true }
        for (let i = 0; i < 40; i += 1) {
          if (!checking && cached !== null) return { ...localize(cached.data, uilang), cached: true }
      ...
      try {
        ...
        return { ...localize(data, uilang), cached: false }
      } finally {
        checking = false
      }
    } catch (error) {
  ...
  const setProgress = (patch) => {
    ...
    // 操作一结束就把运行日志落盘：界面不再显示日志面板，排查靠这个文件
    if (patch.phase === 'done' || patch.phase === 'error') {
      opLogDump(String(progress.type || '') + ' ' + String(progress.phase) + ' ' + String(progress.message || ''))
    }
  }
  ...
  const update = async (uilang) => {
    ...
    updating = true
    opLogReset()
    opLogNote('[update] 开始通过 npm 更新 DSH · ' + new Date().toLocaleString())
    ...
    try {
      ...
      // 先确认插件自身不是旧版：升级 DSH 的动作由**当前运行的插件代码**执行，
      // 旧版插件（如 1.9.0 的 Windows staged 安装缺 -g，交换后会丢掉全部依赖）
      // 本身的升级流程就是危险的 → 拦住并给出确切的插件更新命令。
      const plugin = await pluginInfo()
      if (updateBlockedBySelf(plugin)) {
        const msg = tr(uilang, 'pluginUpdateRequired', { current: plugin.version || '?', latest: plugin.latest })
        setProgress({ type: 'update', phase: 'error', message: msg, detail: plugin.updateCommand })
        return { ok: false, error: msg, pluginOutdated: true, plugin, updateCommand: plugin.updateCommand, mode }
      }
      ...
      // 升级安全网 1：先回收未被 pending-swap 引用的遗留暂存目录
      // （实测 9/4 一次未完成的更新留下 222 MB 暂存目录，永远不会被自动回收）
      const swept = sweepStaleStaging()
      if (swept.count > 0) console.log(tr(uilang, 'stagingSwept', { n: swept.count, mb: formatMb(swept.bytes) }))
      // 升级安全网 2：真正会变版本时，先落一份配置/会话快照（会话格式迁移不可降级）
      let targetVersion = cached !== null && typeof cached.data.remoteVersion === 'string' ? cached.data.remoteVersion : null
      if (targetVersion === null) {
        try { targetVersion = (await remoteVersionOf()).version } catch (e) { targetVersion = null }
      }
      const willChange = targetVersion !== null && (beforeVersion === null || compareVersions(targetVersion, beforeVersion) !== 0)
      let backup = null
      if (willChange || expectUpdate) {
        setProgress({ type: 'update', phase: 'running', message: tr(uilang, 'backingUp') })
        backup = createUpgradeBackup(beforeVersion, targetVersion)
        const pruned = pruneUpgradeBackups()
        if (pruned.count > 0) console.log(tr(uilang, 'backupPruned', { n: pruned.count }))
        if (backup.dir !== null) console.log(tr(uilang, 'backupCreated', { dir: backup.dir }))
        if (backup.dir !== null) opLogNote('[backup] 升级前快照: ' + backup.dir + '（会话 ' + String(backup.sessions) + '）')
      }
      // Windows：更新目标即运行实例自身（含 native 依赖），原地 npm install 会撞
      // EBUSY（用户已遇到：lib 被锁、半拆半装）。改为 staged 安装 + 待交换标记，
      // 由用户点击「重启 DSH」时原子替换并重启（不再自动重启）。
      ...
      const withSafety = {
        ...result,
        plugin,
        backupDir: backup !== null ? backup.dir : null,
        backupSessions: backup !== null ? backup.sessions : null,
        backupSessionsBytes: backup !== null ? backup.sessionsBytes : 0,
        sweptBytes: swept.bytes,
      }
      cached = { data: { ...withSafety, checkedAt: now, installDir, copies }, at: now }
   ...
   * 无 EBUSY）→ 校验 → 写「待交换」标记。**不再自动重启**：由用户点击
   * 「重启 DSH」时按原命令重新拉起脚本完成「交换部署 + 重启」（见 restartNow）。
   */
  const updateStagedFlow = async (installDir, beforeVersion, uilang, expectUpdate) => {
    ...
    writePendingSwap('update', installDir, stagingDir, staged.stagingPkg, staged.version)
    const hint = tr(uilang, 'stagedReady', { version: target })
    push(hint, '')
    return { ok: true, mode: 'npm-global', staged: true, updated: true, needsRestart: true, beforeVersion, version: target, hint }
  }

  /** 修复部署（半拆半装恢复）：staged 重装当前版本 + 写「待交换」标记，手动重启完成。 */
  const repairDeployment = async (uilang) => {
    ...
    opLogReset()
    opLogNote('[repair] 开始重装当前版本 ' + current + ' · ' + new Date().toLocaleString())
    if (process.platform !== 'win32') {
      // 非 Windows：原地 npm 重装当前版本（无 EBUSY 问题；重启后完全生效）
      const res = await runNpmUpdate(installDir, current, null, uilang, false, current)
      if (!res.ok) return { ok: false, error: tr(uilang, 'repairFail') + (res.error || '') }
      return { ok: true, needsRestart: true, hint: tr(uilang, 'repairDone') }
    }
    clearPendingSwap()
    ...
    writePendingSwap('repair', installDir, stagingDir, staged.stagingPkg, staged.version)
    return { ok: true, needsRestart: true, hint: tr(uilang, 'repairDone') }
  }

  /**
   * 重启当前 DSH 实例（跨平台）：生成 detached 启动脚本（Windows 用 PowerShell、
   * macOS/Linux 用 /bin/sh），等当前进程退出后按原启动命令重新拉起 dsh；
   * 若存在「待交换」标记（Windows staged 更新/修复），脚本会先原子替换部署
   * 目录（失败自动恢复旧版）再重启。日志写入 $DSH_HOME/plugin-data/dsh-updater-npm/restart.log。
   */
  const restartNow = async (uilang) => {
    if (restarting) return { ok: false, error: tr(uilang, 'busyRestart') }
    if (updating) return { ok: false, error: tr(uilang, 'busyUpdate') }
    const located = await locateInstall(ctx)
    if (located === null) return { ok: false, error: tr(uilang, 'locateFailShort') }
    restarting = true
    try {
      const pending = readPendingSwap()
      const swap = pending !== null
        ? {
            installDir: pending.installDir,
            stagingPkg: pending.stagingPkg,
            stagingDir: pending.stagingDir,
            version: typeof pending.version === 'string' ? pending.version : null,
          }
        : null
      // 清掉上一次的重启结果，避免 UI 展示陈旧信息
      try { rmSync(restartResultFile(), { force: true }) } catch (e) { /* 忽略 */ }
      const scriptText = buildRestartScript({
        platform: process.platform,
        pid: process.pid,
        nodeExe: process.execPath,
        args: [...process.execArgv, ...process.argv.slice(1)],
        cwd: process.cwd(),
        logFile: join(pluginDataDir(), 'restart.log'),
        swap,
        // 重启后要写回的机器可读结果
        resultFile: restartResultFile(),
        outLog: restartOutFile(),
        errLog: restartErrFile(),
      })
      const logFile = join(pluginDataDir(), 'restart.log')
      const launched = launchRestartScript(scriptText, logFile)
      if (!launched.ok) return { ok: false, error: tr(uilang, 'restartFail') + (launched.error || '') }
      // 关键：确认脚本**真的启动**了再交出暂存包。
      // 旧版在 Windows 上用 detached 起 powershell.exe，脚本一行没跑却返回成功，
      // 还顺手删掉了 pending-swap.json —— 暂存包随后被启动清扫当垃圾删掉，
      // 用户看到的只有「已触发重启」+ 重启后仍是旧版本。
      const started = await waitForBootstrap(logFile, launched.token)
      if (!started) {
        return {
          ok: false,
          error: tr(uilang, 'restartNotStarted'),
          script: launched.script,
          keptPendingSwap: swap !== null,
        }
      }
      // 交换信息已嵌入脚本：只删标记文件防重复触发（暂存目录由交换脚本在交换后清理）
      try { rmSync(pendingSwapFile(), { force: true }) } catch (e) { /* 忽略 */ }
      setProgress({ type: 'restart', phase: 'done', message: tr(uilang, 'restarting'), detail: '', done: 1, total: 1, current: '' })
      return { ok: true, hint: tr(uilang, swap !== null ? 'restartLaunchedSwap' : 'restartLaunched') }
    } finally {
      restarting = false
    }
  }
  ...
  const updateSource = async (uilang) => {
    ...
    updating = true
    opLogReset()
    opLogNote('[source-update] 开始 git 更新源码树 · ' + new Date().toLocaleString())
    ...
    try {
      ...
      // 3) 安装依赖
      const inst = await runDepsInstall(info.root, (d) => push('running', tr(uilang, 'sourceInstall', { cmd: 'pnpm' }), d), uilang)
      if (!inst.ok) {
  ...
  const installPwsh = async (uilang) => {
    ...
    updating = true
    opLogReset()
    opLogNote('[install-pwsh] 开始安装 PowerShell 7 · ' + new Date().toLocaleString())
  ...
  // 进度快照：只回传进度本身。
  // 运行日志面板已按用户要求移除（1.12.3），opLog 只留作失败留痕（last-run.log）。
  ...
  ctx.inject(['webServer'], (hostCtx) => {
    ...
    host.effect(() => host.webServer.register({
      ...
      handler: (req, res) => {
        ...
        const uilang = uilangOf(req)
        respond(req, res, () => restartNow(uilang))
      },
    }), 'dsh-updater-npm: restart route')

    host.effect(() => host.webServer.register({
      kind: 'exact',
      path: '/dsh-updater-npm/cleanup',
      handler: (req, res) => {
        if (req.method !== 'POST') { res.writeHead(405, { allow: 'POST' }); res.end(); return }
        if (!sameOrigin(req)) {
          const text = JSON.stringify({ ok: false, error: 'cross-origin request rejected' })
        ...
        respond(req, res, async () => {
          const located = await locateInstall(ctx)
          const swept = cleanupLeftovers(located !== null ? located.installDir : null)
          // 清完立刻重算缓存，UI 下一次 check 就能看到新状态
          cached = null
          return {
            ok: true,
            files: swept.staging + swept.residuals,
            bytes: swept.bytes + swept.residualBytes,
            hint: tr(uilangOf(req), 'cleanupDone', {
              n: swept.staging + swept.residuals,
              mb: formatMb(swept.bytes + swept.residualBytes),
            }),
          }
        })
      },
    }), 'dsh-updater-npm: cleanup route')

    // 清理回滚点：只有确认新版已经跑起来之后才应该点（客户端按 rollback 字段决定是否显示）
    host.effect(() => host.webServer.register({
      kind: 'exact',
      path: '/dsh-updater-npm/cleanup-rollback',
      handler: (req, res) => {
        if (req.method !== 'POST') { res.writeHead(405, { allow: 'POST' }); res.end(); return }
        if (!sameOrigin(req)) {
          const text = JSON.stringify({ ok: false, error: 'cross-origin request rejected' })
        ...
        respond(req, res, async () => {
          const located = await locateInstall(ctx)
          if (located === null) return { ok: false, error: tr(uilangOf(req), 'locateFailShort') }
          // 双保险：当前运行版本所在的目录永远不会被当成回滚点删掉
          const swept = removeRollbackDirs(located.installDir, localVersionOf(located.installDir))
          cached = null
          return { ok: true, files: swept.files, bytes: swept.bytes }
        })
      },
    }), 'dsh-updater-npm: cleanup rollback route')
  ...
  // ── 定时任务 ──────────────────────────────────────────────────────────────
  // 启动后台清理一次遗留暂存目录（未完成更新留下的垃圾，实测有 222 MB 级）
  void (async () => {
    try {
      await ctx.timer.timeout(8000)
      const swept = sweepStaleStaging()
      if (swept.count > 0) console.log(tr(SYSTEM_LANG, 'stagingSwept', { n: swept.count, mb: formatMb(swept.bytes) }))
    } catch (e) { /* 静默 */ }
  })()
...
export const __test = {
  ...
  formatMb,
  dirSize,
  stagingLayoutMatches,
  verifyStagedBinary,
  listStagingWaste,
  summarizeStagingWaste,
  sweepStaleStaging,
  listNpmResiduals,
  summarizeNpmResiduals,
  listRollbackDirs,
  summarizeRollbackDirs,
  removeRollbackDirs,
  cleanupLeftovers,
  createUpgradeBackup,
  pruneUpgradeBackups,
