...
/** 执行 npm 全局升级，完成后回读版本。 */
function runNpmUpdate(installDir, beforeVersion, onProgress = null, uilang) {
  return new Promise((resolve) => {
    ...
    const pushProgress = () => {
      if (onProgress) {
        ...
        onProgress({ phase: 'running', done: 0, total: 0, current: '', message: tr(uilang, 'npmRunning'), detail })
      }
    }
    ...
    child.on('close', (code) => {
      ...
      resolve({
        ...
        hint: code === 0 ? (updated ? tr(uilang, 'updatedHint', { version: afterVersion }) : tr(uilang, 'upToDateHint')) : null,
      })
    })
  })
}
...
export function apply(ctx) {
  ...
  const update = async (uilang) => {
    ...
    try {
      ...
      setProgress({ type: 'update', phase: 'running', message: tr(uilang, 'npmRunning') })
      const result = await runNpmUpdate(installDir, beforeVersion, (p) => setProgress({ type: 'update', ...p }), uilang)
      const now = Date.now()
