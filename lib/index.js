...
/** 执行一条安装命令（winget / msiexec / npm 兜底），流式回传输出尾部。 */
function runInstallCmd(argv, onProgress, timeoutMs = 10 * 60 * 1000) {
  return new Promise((resolve) => {
    ...
    let child
    try {
      child = spawn(argv[0], argv.slice(1), { stdio: ['ignore', 'pipe', 'pipe'], shell: needShell })
    } catch (e) {
...
async function installStaged(installDir, targetVersion, stagingDir, uilang, push) {
  const own = npmCliFor(installDir)
  const base = ['install', '--prefix', stagingDir, '--no-save', '--no-audit', '--no-fund', '--install-strategy', 'nested', '@deepseek-ai/dsh@' + targetVersion]
  const argv = own !== null
    ? [own.nodeExe, own.npmCli, ...base]
    : ['npm', ...base]
...
function writePendingSwap(kind, installDir, stagingDir, stagingPkg) {
  writeJson(pendingSwapFile(), { kind, installDir, stagingDir, stagingPkg, createdAt: Date.now() })
}
...
function buildRestartScript(opts) {
  const { platform, pid, nodeExe, args, cwd, logFile, swap } = opts
  ...
  if (platform === 'win32') {
    ...
    if (swap && typeof swap.stagingPkg === 'string') {
      // 先校验暂存新包：不通过则放弃整个操作（当前进程继续运行），避免「杀完进程却换不了」
      lines.push(
        "if (-not (Test-Path " + psq(join(swap.stagingPkg, 'package.json')) + ") -or -not (Test-Path " + psq(join(swap.stagingPkg, 'lib', 'bin.js')) + ")) { Log 'staging invalid, aborting (no swap, no restart)'; exit 1 }")
    }
    lines.push(
    ...
    if (swap && typeof swap.stagingPkg === 'string') {
      ...
      lines.push(
        // 原子替换 + 失败恢复（旧目录改名备份，新包移入，校验失败自动恢复）
        "$old = '" + inst.replace(/'/g, "''") + ".old-' + [DateTime]::Now.ToString('yyyyMMddHHmmss')",
        'try {',
        "  Rename-Item -Path '" + inst.replace(/'/g, "''") + "' -NewName $old",
        "  Move-Item -Path '" + stg.replace(/'/g, "''") + "' -Destination '" + inst.replace(/'/g, "''") + "'",
        "  if (-not (Test-Path '" + inst.replace(/'/g, "''") + "\\package.json') -or -not (Test-Path '" + inst.replace(/'/g, "''") + "\\lib\\bin.js')) { throw 'post-swap check failed' }",
        "  Remove-Item -Path $old -Recurse -Force -ErrorAction SilentlyContinue",
        "  Log 'swap OK'",
        '} catch {',
        "  Log ('swap failed: ' + $_.Exception.Message + '; restoring old install')",
        "  Remove-Item -Path '" + inst.replace(/'/g, "''") + "' -Recurse -Force -ErrorAction SilentlyContinue",
        "  Rename-Item -Path $old -NewName '" + inst.replace(/'/g, "''") + "'",
        "  Log 'restored old install'",
        '}',
        "Remove-Item -Path '" + sd.replace(/'/g, "''") + "' -Recurse -Force -ErrorAction SilentlyContinue",
      )
    }
    lines.push(
      "Log 'relaunching dsh'",
      'Start-Process -FilePath ' + psq(nodeExe) + ' -ArgumentList @(' + argList.map(psa).join(', ') + ') -WorkingDirectory ' + psq(cwd),
      "Log 'relaunch issued'",
    )
    return lines.join('\n')
  }
  ...
  if (swap && typeof swap.stagingPkg === 'string') {
    lines.push(
      "if [ ! -f " + shq(join(swap.stagingPkg, 'package.json')) + " ] || [ ! -f " + shq(join(swap.stagingPkg, 'lib', 'bin.js')) + " ]; then log 'staging invalid, aborting (no swap, no restart)'; exit 1; fi")
  }
  lines.push(
    'sleep 2',
    'PID=' + String(pid),
  ...
  if (swap && typeof swap.stagingPkg === 'string') {
    ...
    lines.push(
      ...
      'SD=' + shq(sd),
      'if mv -f ' + shq(inst) + ' "$OLD"; then',
      '  if mv -f ' + shq(stg) + ' ' + shq(inst) + " && [ -f " + shq(join(inst, 'package.json')) + ' ]; then',
      '    rm -rf "$OLD"',
      "    log 'swap OK'",
      '  else',
      "    log 'swap failed; restoring old install'",
      '    rm -rf ' + shq(inst),
      '    mv -f "$OLD" ' + shq(inst),
      "    log 'restored old install'",
      '  fi',
      'else',
      "  log 'move old failed; keeping current install'",
      'fi',
      'rm -rf "$SD"',
    )
  }
  lines.push(
    "log 'relaunching dsh'",
    'cd ' + shq(cwd) + ' || exit 1',
    'nohup ' + shq(nodeExe) + (argList.length > 0 ? ' ' + argList.map(shq).join(' ') : '') + ' >> ' + shq(logFile) + ' 2>&1 &',
    "log 'relaunch issued'",
  )
  return lines.join('\n')
}

/** 把重启脚本写入插件数据目录并启动（当前进程退出后继续执行）。 */
function launchRestartScript(scriptText) {
  ...
  try {
    ...
    const cmdArgs = win
      ...
      : [file]
    // 注意：Windows 上不要用 detached:true —— Node 在 Windows 以
    // CREATE_NEW_PROCESS_GROUP 启动 powershell.exe 时出现「进程 spawn 成功但脚本
    // 从未执行」的现象（已复现：无日志、无输出、exit 0）。脚本自身的职责是
    // Stop-Process 当前 dsh 进程再拉起新进程；Windows 强制杀父进程不会级联杀死
    // 其子进程，所以非 detached + unref 即能保证脚本在 dsh 退出后继续执行。
    const child = spawn(win ? 'powershell.exe' : '/bin/sh', cmdArgs, { stdio: 'ignore', shell: false })
    child.unref()
...
export function apply(ctx) {
  ...
  const updateStagedFlow = async (installDir, beforeVersion, uilang, expectUpdate) => {
    ...
    writePendingSwap('update', installDir, stagingDir, staged.stagingPkg)
  ...
  const repairDeployment = async (uilang) => {
    ...
    writePendingSwap('repair', installDir, stagingDir, staged.stagingPkg)
    return { ok: true, needsRestart: true, hint: tr(uilang, 'repairDone') }
  }
  ...
  const restartNow = async (uilang) => {
    ...
    try {
      ...
      const swap = pending !== null ? { installDir: pending.installDir, stagingPkg: pending.stagingPkg, stagingDir: pending.stagingDir } : null
      ...
      return { ok: true, hint: tr(uilang, 'restartLaunched') }
    } finally {
      restarting = false
    }
  }
