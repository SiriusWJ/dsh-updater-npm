...
const HOST_I18N = {
  zh: {
    ...
    updateStagingStalled: 'npm 连续 {min} 分钟无任何输出，判定卡死并已终止（本次已运行 {sec}）。若是网络太慢，请调大 config.json 里的 npmIdleMinutes',
    updateStagingTimeout: 'npm 安装超过总时长上限 {min} 分钟被终止（本次已运行 {sec}）。请调大 config.json 里的 npmTimeoutMinutes 后重试',
    ...
    restartFail: '重启失败: ',
    restartNotStarted: '重启脚本没有真正启动（脚本未执行、当前进程也未被结束）——部署与暂存包都保持原样。请手动重启 DSH，重启后再点一次「重启 DSH」即可完成交换。',
    ...
    restartLaunchedSwap: '✅ 已触发重启：DSH 将退出，完成版本交换后自动重启；浏览器需要用新弹出的激活地址重新打开。',
    breakingTitle: '跨版本升级注意',
    breakingKnown: '0.1.5 起：会话格式升级到 V3（迁移后不可降级读取）；自定义 persona 配置由 text 改为 prefix/suffix；插件 API 与槽位变更（详情面板 details.* 移除、顶层 conversation 迁到 main.conversation、移除 ctx.agent、Inbox 接口调整）。',
    breakingGeneric: '本次跨 {n} 个小版本，升级前请先阅读 release notes 与插件兼容性说明。',
    ...
    activationHint: '新激活地址（重启后浏览器需用它重新授权）：',
    openBtn: '打开',
  ...
  en: {
    ...
    updateStagingStalled: 'npm produced no output for {min} min — treated as stalled and terminated (ran {sec}). Raise npmIdleMinutes in config.json if your network is slow',
    updateStagingTimeout: 'npm exceeded the {min} min total limit and was terminated (ran {sec}). Raise npmTimeoutMinutes in config.json and retry',
    ...
    restartNotStarted: 'The restart script never actually started (nothing ran, this process was not stopped) — the deployment and the staged package are untouched. Restart DSH manually, then click "Restart DSH" once more to finish the swap.',
    ...
    restartLaunchedSwap: '✅ Restart triggered — DSH will exit, swap the version, and relaunch itself; the browser must reopen the new activation URL that pops up.',
    breakingTitle: 'Cross-version upgrade notice',
    breakingKnown: 'Since 0.1.5: the session format moves to V3 (no downgrade reads after migration); a custom persona config moves from text to prefix/suffix; plugin APIs and slots changed (details.* removed, top-level conversation moved to main.conversation, ctx.agent removed, Inbox reshaped).',
    breakingGeneric: 'This jump spans {n} minor versions — read the release notes and plugin compatibility notes first.',
    ...
    activationHint: 'New activation URL (the browser must use it to re-authenticate after a restart):',
    openBtn: 'Open',
...
/** 重启后的新激活地址文件（重启脚本抓取）。 */
function activationUrlFile() {
  return join(pluginDataDir(), 'activation-url.txt')
}
...
function readRestartResult() {
  ...
  const url = typeof r.url === 'string' && r.url !== ''
    ? r.url
    : (existsSync(activationUrlFile()) ? readFileSync(activationUrlFile(), 'utf8').trim() || null : null)
  return {
...
function buildRestartScript(opts) {
  const { platform, pid, nodeExe, args, cwd, logFile, swap, resultFile, outLog, errLog, urlFile } = opts
  ...
  const wantUrl = typeof urlFile === 'string' && urlFile !== ''
  if (platform === 'win32') {
    // PowerShell 单引号转义：' → ''；整体含空格/括号时外包双引号防止 Start-Process 拆参
    const psq = (s) => {
    ...
    if (hasResult) {
      lines.push(
        "$result = [ordered]@{ at = (Get-Date -Format o); swapped = $false; swapOk = $false; version = " + psq(wantSwap && swap.version ? swap.version : '') + "; oldDir = ''; url = ''; error = '' }",
    ...
    // 抓取新的激活地址：0.1.5 起 Web 鉴权 cookie 由「本次激活」的密钥签名，
    // 重启后旧 cookie 失效，必须把新地址交给用户（旧流程会把它丢掉）。
    if (wantOut) {
      lines.push(
        "$url = ''",
        'for ($i = 1; $i -le 40; $i++) {',
        '  Start-Sleep -Seconds 3',
        '  if (Test-Path ' + psq(outLog) + ') {',
        "    $m = Select-String -Path " + psq(outLog) + " -Pattern 'http://[^\\s]*token=[A-Za-z0-9_\\-]+' -ErrorAction SilentlyContinue | Select-Object -Last 1",
        '    if ($m) { $url = $m.Matches[0].Value; break }',
        '  }',
        '}',
        '$url = [string]$url',
        "if ($url -ne '') {",
        ...(wantUrl ? ['  try { [IO.File]::WriteAllText(' + psq(urlFile) + ', $url) } catch {}'] : []),
        ...(hasResult ? ['  $result.url = $url'] : []),
        "  Log ('activation url: ' + $url)",
        "  try { Start-Process $url; Log 'opened the activation url in the default browser' } catch { Log 'could not open the activation url' }",
        '} else {',
        "  Log 'activation url not observed; see the restart log'",
        '}',
        ...(hasResult ? ['WriteResult'] : []),
      )
    }
  ...
  if (wantResult) {
    lines.push(
      ...
      'write_result() { printf \'{"at":"%s","swapped":%s,"swapOk":%s,"version":"%s","oldDir":"%s","url":"%s","error":"%s"}\\n\' "$(date -u +%FT%TZ)" "$SWAPPED" "$SWAPOK" "$VERSION" "$OLDDIR" "$URL" "$ERR" > "$RESULT_FILE" 2>/dev/null; }')
  }
  if (wantSwap) {
  ...
  if (wantOut) {
    lines.push(
      ...
      'while [ "$i" -lt 40 ]; do',
      '  sleep 3',
      '  URL=$(grep -o ' + shq('http://[^ ]*token=[A-Za-z0-9_-]*') + ' ' + shq(outLog) + ' 2>/dev/null | tail -1)',
      '  [ -n "$URL" ] && break',
      '  i=$((i + 1))',
      'done',
      'if [ -n "$URL" ]; then',
      ...(wantUrl ? ['  printf "%s" "$URL" > ' + shq(urlFile) + ' 2>/dev/null'] : []),
      "  log 'activation url captured'",
      // 尽力打开默认浏览器（macOS / Linux 各自尽力而为）
...
export function apply(ctx) {
  ...
  const restartNow = async (uilang) => {
    ...
    try {
      ...
      const swap = pending !== null
        ...
        : null
      // 清掉上一次的重启结果/激活地址，避免 UI 展示陈旧信息
      try { rmSync(restartResultFile(), { force: true }) } catch (e) { /* 忽略 */ }
      try { rmSync(activationUrlFile(), { force: true }) } catch (e) { /* 忽略 */ }
  ...
  // 进度快照：附带操作日志增量（since=客户端已持有的行号游标）与当前超时设置。
  const progressSnapshot = (since = 0) => ({
    ...progress,
    log: opLogSince(since),
    limits: readPluginConfig(),
  })
  ...
  ctx.inject(['webServer'], (hostCtx) => {
    ...
    host.effect(() => host.webServer.register({
      ...
      handler: (req, res) => {
        ...
        respond(req, res, () => progressSnapshot(Number.isFinite(since) && since > 0 ? since : 0))
      },
