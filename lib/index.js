...
function runInstallCmd(argv, onProgress, timeoutMs = 10 * 60 * 1000) {
  return new Promise((resolve) => {
    ...
    child.stdout.on('data', (chunk) => { stdout = (stdout + chunk.toString()).slice(-8192); push() })
    child.stderr.on('data', (chunk) => { stderr = (stderr + chunk.toString()).slice(-8192); push() })
    child.on('error', (e) => { clearTimeout(timer); resolve({ ok: false, error: String((e && e.message) || e), detail: (stderr || stdout).trim().slice(-400) }) })
    child.on('close', (code) => { clearTimeout(timer); resolve({ ok: code === 0, code, detail: (stderr || stdout).trim().slice(-400), stdout: stdout.trim().slice(-600), stderr: stderr.trim().slice(-600) }) })
  })
}
